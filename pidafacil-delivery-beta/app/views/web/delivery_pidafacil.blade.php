...
@section('content')
    ...
    <div class="container-fluid gray_content">
        <div class="row">
            ...
            <a href="?fillter=2" ><div class="col-lg-3 col-md-3 col-sm-4 col-xs-12">
                    <div class="panel panel-cashed">
                        <div class="panel-heading">
                            <div class="row">
                                <div class="col-xs-3">
                                ...
                                <div class="col-xs-9 text-right">
                                    <div class="huge" id="pending">{{ $stats['registered']['fillter'] }}</div>
                                    <div>Cobradas</div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="panel panel-process">
                        <div class="panel-heading">
                            <div class="row">
                                ...
                                <div class="col-xs-9 text-right">
                                    <div class="huge" id="pending">{{ $stats['accepted']['fillter'] }}</div>
                                    <div>Por recoger</div>
                                </div>
                            </div>
                        </div>
        ...
        <div class="row">
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                <div class="panel panel-default">
                    <div class="panel-heading">
                        <i class="fa fa-motorcycle fa-fw" style="color:#333333;"></i> Ordenes
                    </div>
                    <!-- /.panel-heading -->
                                ...
                                @if(isset($orders))
                                    @foreach($orders as $key=>$order)
                                        @foreach($order->users as $ke=>$val)
                                            ...
                                            ?>
                                        @endforeach
                                        @if(isset($order->statusLogs) && count($order->statusLogs) > 0)

                                            @foreach($order->statusLogs as $ke=>$val)
                                                <?php
                                                $status = $val;
                                                ?>
                                            @endforeach
                                                ...
                                                @if($status->order_status_id == $idF)
                                                    @if(isset($_GET['forma']) && $order->service_type_id == $_GET['forma'])
                                                        @if($co < $hasta and $co >= $desde)
                                                            @include('include.elements_delivery_pidafacil')
                                                        @endif
                                                        <?php $co=$co + 1; ?>
                                                    @elseif(!isset($_GET['forma']))
                                                        @if($MasDeUnDespacho[$order->id])
                                                            @if($co < $hasta and $co >= $desde)
                                                                @include('include.elements_delivery_pidafacil')
                                                            @endif
                                                            <?php $co=$co + 1; ?>
        ...
        <script>
            ...
            socket.on('delivery_pidafacil', function(msg){
