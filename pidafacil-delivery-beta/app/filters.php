...
Route::filter('auth_visor', function()
{
    if (Auth::guest())
    {
        // Save the attempted URL
...
Route::filter('role_visor', function()
{
    ...
    $resId=DB::table('res_user')
        ...
        ->where('user_id',Auth::user()->user_id)
        ->get();
    //variable con se utiliza como contador/vandera.
    $con=0;
    ...
    if (Auth::user()->role_id > 0 && Auth::user()->role_id == 2) {
        ...
        //Role 4, Delivery pidafacil
    }elseif(Auth::user()->role_id > 0 && Auth::user()->role_id == 4){
        if($arrurl[$conturl] == "delivery_pidafacil")
        {
...
Route::filter('role_pidafacil', function()
{
    if(Auth::guest())
    {
        return Redirect::guest('restauran_login');
