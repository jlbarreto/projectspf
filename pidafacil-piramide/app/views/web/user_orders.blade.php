    ...
    <div class="center_content">
        ...
        <div class="row">
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 space">
                ...
                <?php
                $idStatusOrder = 0;
                $arrMensajes = array("Pendiente", "Pendiente", "Registrada", "Aceptada", "Despachada", "Entregada", "Cancelada", "Rechazada", "Incobrable");
                $arrClassCss = array("order_pendiente", "order_pendiente", "order_registrada", "order_aceptada", "order_despachada", "order_entregada", "order_cancelada", "order_rechazad", "order_incobrable");
                ?>
                @for($i = 0; $i < count($last_order->statusLogs); $i++)
<?php $idStatusOrder = $last_order->statusLogs[$i]->order_status_id; ?>
                @endfor
                <div class="order_completed {{$arrClassCss[$idStatusOrder]}} order_estado">
                    <p>Estado: {{$arrMensajes[$idStatusOrder]}} @if($idStatusOrder == 1)
                        <button style="  display: table; margin-top: 10px;" class="btn btn-default" data-target="#cancel_this" data-toggle="modal" >Cancelar</button>@endif</p>
                </div>
...
<!-- Form confirm (yes/ok) handler, submits form -->
                        ...
                        $(".ejectar_rejected").on('click', function () {
                            ...
                            url: '../restaurant-orders/cancel',
                                    type: 'post',
                                    dataType: 'json',
                                    data: {idA: {{$last_order->order_id}}, comment:comment, rejected:rejected, id_rest: {{$last_order->restaurant_id}}, motivoRechazo:motivoRechazo}
                })
