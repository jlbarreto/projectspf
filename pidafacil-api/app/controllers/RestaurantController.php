...
class RestaurantController extends \BaseController {
  ...
  */
  public function sections() {
    ...
    try {
      $statusCode = 200;
      $sections = Section::where('restaurant_id', $input['restaurant_id'])
      ->where('activate', true)
      ->get();
      if (!empty($sections)) {
        $response['status'] = true;
  ...
  */
  public function products() {
    ...
    try {
      ...
      if (isset($input['page_size']) && $input['page_size'] > 0) {
        $pagesze = $input['page_size'];
        $pagepos = $input['page_post'];
        $products = Product::where('section_id', $input['section_id'])
        ->where('activate', true)
        ->take($pagesze)->skip($pagepos * $pagesze)
        ->get();
      } else {
  ...
  public function addresses() {
    ...
    try {
      $statusCode = 200;
      $limit = Restaurant::where("parent_restaurant_id", $input['restaurant_id'])
      ->where("activate", 1)
      ->count();
      if ($limit > 0) {
        $addrs = Restaurant::where("parent_restaurant_id", $input['restaurant_id'])
        ->where("activate", 1)
        ->get();
        foreach ($addrs as $key => $value) {

          //Solamente cargar cuando sea un solo restaurante, sino sucurusales y no marca
          if ($limit == 1 or $value->restaurant_id != $input['restaurant_id']) {
            $resadds[] = array(
              "restaurant_id" => $value->restaurant_id,
              "address" => $value->name . " - " . $value->address
            );
