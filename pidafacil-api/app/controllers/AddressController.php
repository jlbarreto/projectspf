...
//////////////////////////////////////////////////////
              ...
              log::info('SI dentro trabajo '.$hoy);
               ...
               $hoy2 = date("w"); 
                ...
                $zonesLst= DB::select('select z.zone, z.zone_id, a.address_id,
                    ...
                    WHERE  z.active = 1 and t.day_id = "'.$hoy2.'" and a.user_id = "'.$input['user_id'].'"
            ...
            $zonesLst= DB::select('select z.zone, a.address_name, a.address_id, a.address_1, a.address_2, a.reference, z.zone_id, 
                       ...
                       z.shipping_price_id,
                       p.price prom_time
                        FROM   pf.com_zones z INNER JOIN pf.com_shipping_prices p ON z.shipping_price_id = p.shipping_price_id Left join pf.diner_addresses a on z.zone_id = a.zone_id WHERE  z.active = 1  and a.user_id = "'.$input['user_id'].'" 
                 ORDER  BY z.zone'); 
                     foreach ($zonesLst as $z) {
                    $lst[] = array(
...
/////////////////////////////////////
            $statusCode = 200;
            $limit = Address::where('user_id', $input['user_id'])->count();
            $numDirecciones = $limit;
           
        
            if ($limit > 0) {
                $addrs = Address::where('user_id', $input['user_id'])->get();
                ...
                $response = array(
                    "status" => false,
                    "data" => 'No hay direcciones registradas.'
                );
            }
        } catch (Exception $e) {
            $statusCode = 400;
...
///////////7
    ...
    public function show() {
        ...
        try {
            $statusCode = 200;
            $addr = Address::findOrFail($input['address_id']);
