...
$(document).ready(function(){
  ...
  $(".ejectar_rejected").on('click', function(event){
    var id_elemento = $(this).attr('id');
    var id_pedido = id_elemento.split("_");
    ...
    var motivo = $("#motivoRechazo_"+id_pedido[2]+" option:selected").val();
    var url = $("#urlCancel_"+id_pedido[2]).val();

    if(rejected == '7' && motivo != 'none' && comment != ''){
      $.ajax({
        url: url,
        type: 'post',
        dataType: 'json',
  ...
  $(".add_note").on('click', function(event){
    ...
    if(motivo == 'Producto Agregado'){
      var nombrePro = $("input[name='nombrePro[]']").map(function(){return this.value;}).get();
      var precioPro = $("input[name='precioPro[]']").map(function(){return this.value;}).get();
      var precioProV = $("input[name='precioProV[]']").map(function(){return this.value;}).get();
    }

    if(motivo != 'none') {
      if(motivo == 'Producto Agregado' && (nombrePro != '' && precioPro != '' && precioProV != '')){
        $btn.button('loading');
        $.ajax({
          url: 'comment',
          type: 'post',
      ...
      }else if(motivo != 'Producto Agregado'){
        $btn.button('loading');
        $.ajax({
          url: 'comment',
          type: 'post',
        ...
        alert("Debe especificar los productos y precios.");
        event.preventDefault();
      }
    }else{
  ...
  $(".note_by").on('click', function(){
    ...
    if(id_pedido != ''){
      ...
      .done(function (result){
        ...
        $("#num_total2_"+id_pedido[2]+"").text(result);
      })
      .fail(function (result){
        $btn.button('reset');
        alert("No se pudo realizar la acción, por favor intenta de nuevo");
  ...
  $(".actualizarOrd").on('click', function(event){
    var $btn = $(this);
    var id_elemento = $(this).attr('id');
    var id_pedido = id_elemento.split("_");
    ...
    if($("#zoneid_"+id_pedido[1]+" option:selected").val() == 'none'){
      event.preventDefault();
      alert("Debe elegir una zona antes de modificar la orden");
    }else{
      ...
      if(estadoNew == 6 || estadoNew == 7){
        $('#editOrden_'+id_pedido[1]).modal('toggle');
        $('#cancel_this_'+id_pedido[1]).modal('show');
      }else{
        $btn.button('loading');
        $.ajax({
          url: 'editar_pedido',
