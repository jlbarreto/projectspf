<!DOCTYPE html>
<html lang="es">
    <head>
        <meta charset="utf-8">
        <meta http-equiv="X-UA-Compatible" content="IE=edge">
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <meta name="description" content="">
        <meta name="author" content="">

        <title>Carrier Pidafacil</title>

        <!-- /#wrapper -->
        <!-- jQuery -->
        {{HTML::script('js/jquery.js')}}
        <!--Scripts que se usan para los reportes-->
        
        <script type="text/javascript" src="../public/js/scripts.js"></script>
        <!-- Menu Toggle Script -->
        {{ HTML::script('js/jquery-1.11.2.min.js') }}
        {{ HTML::script('js/jquery-ui-1.11.4.min.js') }}
        <!-- Bootstrap Core JavaScript -->
        {{HTML::script('js/bootstrap.js')}}
        {{HTML::script('js/bootstrap.min.js')}}
        {{HTML::script('https://cdnjs.cloudflare.com/ajax/libs/underscore.js/1.8.3/underscore-min.js')}}
        
        {{ HTML::style('css/menu.css') }}

        <!-- Bootstrap Core CSS -->
        {{ HTML::style('css/bootstrap.min.css') }}

        <!-- Custom CSS -->
        {{ HTML::style('css/simple-sidebar.css') }}
        {{ HTML::style('css/boobstrap/font-awesome.min.css') }}

        {{HTML::style('css/jquery.datetimepicker.css')}}
        {{ HTML::script('js/jquery.datetimepicker.js', array("type" => "text/javascript")) }}

    </head>
    <body>
        <?php 
            $role = DB::table('assigned_roles')
                ->select('role_id')
                ->where('user_id', Auth::user()->user_id)
                ->get();

            if (isset($role[0])) {
                Auth::user()['role_id'] = $role[0]->role_id;
            }else{
                Auth::user()['role_id'] = 0;
            }
        ?>
        <div id="wrapper">
            <!-- Sidebar -->
            @if($role[0]->role_id == '8')
                <div id="sidebar-wrapper">
                    <ul class="sidebar-nav">
                        <li class="sidebar-brand">
                            <h4 style="color:white;">Visor Delivery</h4>
                        </li>
                        <li>
                            <a href="delivery_pidafacil">Principal</a>
                        </li>
                        <!--<li>
                            <a href="delivery_reportes">Reportes</a>
                        </li>-->
                        <li></li>
                        <li>
                            <a href="ventas">Reporte de Ventas</a>
                        </li>
                        <li></li>
                        <li>
                            <a href="ventas_detalle">Reporte Detalle</a>
                        </li>
                        <li>
                            <a href="reporte_iva">Reporte IVA</a>
                        </li>
                        <li></li>
                        <li>
                            <a href="rest_pago">Reporte nuevo</a>
                        </li>
                        <li></li>
                        <li>
                            <a href="exportar_prod">Productos</a>
                        </li>
                         <li></li>
                        <li>
                            <a id="btnConfig">Configuración</a>
                            <!--<a href="config">Configuración</a>-->
                            <ul id="submenu" style="display:none; color:white;">
                                <li><a href="config">Subir CSV</a></li>
                                <li><a href="promo">Promoción</a></li>
                            </ul>
                        </li>
                        <li></li>
                        <li>
                            <a href="logout">Cerrar Sesión</a>
                        </li>                        
                    </ul>
                </div>
            @else
                <div id="sidebar-wrapper">
                    <ul class="sidebar-nav">
                        <li class="sidebar-brand">
                            <h4 style="color:white;">Visor Delivery</h4>
                        </li>                        
                        <!--<li>
                            <a href="delivery_reportes_rest">Reporte Pedidos</a>
                        </li>
                        <li></li>-->
                        <li>
                            <a href="ventas_rest">Reporte de Ventas</a>
                        </li>
                        <li></li>
                        <li>
                            <a href="ventas_detalle_rest">Reporte Detalle</a>
                        </li>
                        <li>
                            <a href="reporte_tiempo_rest">Reporte Tiempos</a>
                        </li>
                        <li></li>
                        <li>
                            <a href="logout">Cerrar Sesión</a>
                        </li>
                    </ul>
                </div>
            @endif 
            <!-- /#sidebar-wrapper -->
            <!-- Page Content -->
            <div id="page-content-wrapper">
                <div class="container-fluid">
                    <div class="row">
                        <div class="col-lg-12">
                            <a href="#menu-toggle" class="fa fa-bars btn btn-default" id="menu-toggle" style="margin-left: -34px; margin-top: -30px;"></a>
                            @yield('content')
                        </div>
                    </div>
                </div>
            </div>
            <!-- /#page-content-wrapper -->
        </div>
        
        <link rel="stylesheet" href="//code.jquery.com/ui/1.11.4/themes/smoothness/jquery-ui.css">
        <script src="//code.jquery.com/ui/1.11.4/jquery-ui.js"></script>
        <!-- cdn for modernizr, if you haven't included it already -->
        <script src="http://cdn.jsdelivr.net/webshim/1.12.4/extras/modernizr-custom.js"></script>
        <!-- polyfiller file to detect and load polyfills -->
        <script src="http://cdn.jsdelivr.net/webshim/1.12.4/polyfiller.js"></script>
        
        <script>

        $("#btnConfig").click(function(){
            $("#submenu").css("display", "block");
        });


        
        webshims.setOptions('waitReady', false);
        webshims.setOptions('forms-ext', {types: 'datetime-local'});
        webshims.polyfill('forms forms-ext');

        $("#menu-toggle").click(function(e) {
            e.preventDefault();
            $("#wrapper").toggleClass("toggled");
        });

        /*$(function(){
            $("#datepicker1").datepicker({
                format: "yyyy-mm-dd"
            });
        });

        $(function(){
            $("#datepicker2").datepicker({
                format: "yyyy-mm-dd"
            });
        });*/

        $(document).ready(function() {
            tiempo_orden();

            $("#datepicker1").datetimepicker({
                format:'Y-m-d H:i:s',
                lang:'es'
            });

            $("#datepicker2").datetimepicker({
                format:'Y-m-d H:i:s',
                lang:'es'
            });

            $("#datepicker11").datetimepicker({
                format:'Y-m-d H:i:s',
                lang:'es'
            });

            $("#datepicker22").datetimepicker({
                format:'Y-m-d H:i:s',
                lang:'es'
            });

            $('#mespicker').datepicker({
                changeMonth: true,
                changeYear: true,
                showButtonPanel: true,
                dateFormat: 'mm-yy'
            }).focus(function() {
                var thisCalendar = $(this);
                $('.ui-datepicker-calendar').detach();
                $('.ui-datepicker-close').click(function() {
                    var month = $("#ui-datepicker-div .ui-datepicker-month :selected").val();
                    var year = $("#ui-datepicker-div .ui-datepicker-year :selected").val();
                    thisCalendar.datepicker('setDate', new Date(year, month, 1));
                });
            });

            //Estos son los datepicker para el nuevo reporte - 06 abril
            $("#datepicker_r").datetimepicker({
                format:'Y-m-d H:i:s',
                lang:'es'
            });

            $("#datepicker_r2").datetimepicker({
                format:'Y-m-d H:i:s',
                lang:'es'
            });

        });

        /*DATE FORMAT*/
        var dateFormat = function () {
        var    token = /d{1,4}|m{1,4}|yy(?:yy)?|([HhMsTt])\1?|[LloSZ]|"[^"]*"|'[^']*'/g,
            timezone = /\b(?:[PMCEA][SDP]T|(?:Pacific|Mountain|Central|Eastern|Atlantic) (?:Standard|Daylight|Prevailing) Time|(?:GMT|UTC)(?:[-+]\d{4})?)\b/g,
            timezoneClip = /[^-+\dA-Z]/g,
            pad = function (val, len) {
                val = String(val);
                len = len || 2;
                while (val.length < len) val = "0" + val;
                return val;
            };
    
        // Regexes and supporting functions are cached through closure
        return function (date, mask, utc) {
            var dF = dateFormat;
    
            // You can't provide utc if you skip other args (use the "UTC:" mask prefix)
            if (arguments.length == 1 && Object.prototype.toString.call(date) == "[object String]" && !/\d/.test(date)) {
                mask = date;
                date = undefined;
            }
    
            // Passing date through Date applies Date.parse, if necessary
            date = date ? new Date(date) : new Date;
            if (isNaN(date)) throw SyntaxError("invalid date");
    
            mask = String(dF.masks[mask] || mask || dF.masks["default"]);
    
            // Allow setting the utc argument via the mask
            if (mask.slice(0, 4) == "UTC:") {
                mask = mask.slice(4);
                utc = true;
            }
    
            var    _ = utc ? "getUTC" : "get",
                d = date[_ + "Date"](),
                D = date[_ + "Day"](),
                m = date[_ + "Month"](),
                y = date[_ + "FullYear"](),
                H = date[_ + "Hours"](),
                M = date[_ + "Minutes"](),
                s = date[_ + "Seconds"](),
                L = date[_ + "Milliseconds"](),
                o = utc ? 0 : date.getTimezoneOffset(),
                flags = {
                    d:    d,
                    dd:   pad(d),
                    ddd:  dF.i18n.dayNames[D],
                    dddd: dF.i18n.dayNames[D + 7],
                    m:    m + 1,
                    mm:   pad(m + 1),
                    mmm:  dF.i18n.monthNames[m],
                    mmmm: dF.i18n.monthNames[m + 12],
                    yy:   String(y).slice(2),
                    yyyy: y,
                    h:    H % 12 || 12,
                    hh:   pad(H % 12 || 12),
                    H:    H,
                    HH:   pad(H),
                    M:    M,
                    MM:   pad(M),
                    s:    s,
                    ss:   pad(s),
                    l:    pad(L, 3),
                    L:    pad(L > 99 ? Math.round(L / 10) : L),
                    t:    H < 12 ? "a"  : "p",
                    tt:   H < 12 ? "am" : "pm",
                    T:    H < 12 ? "A"  : "P",
                    TT:   H < 12 ? "AM" : "PM",
                    Z:    utc ? "UTC" : (String(date).match(timezone) || [""]).pop().replace(timezoneClip, ""),
                    o:    (o > 0 ? "-" : "+") + pad(Math.floor(Math.abs(o) / 60) * 100 + Math.abs(o) % 60, 4),
                    S:    ["th", "st", "nd", "rd"][d % 10 > 3 ? 0 : (d % 100 - d % 10 != 10) * d % 10]
                };
    
                return mask.replace(token, function ($0) {
                    return $0 in flags ? flags[$0] : $0.slice(1, $0.length - 1);
                });
            };
        }();
    
        // Some common format strings
        dateFormat.masks = {
            "default":      "ddd mmm dd yyyy HH:MM:ss",
            shortDate:      "m/d/yy",
            mediumDate:     "mmm d, yyyy",
            longDate:       "mmmm d, yyyy",
            fullDate:       "dddd, mmmm d, yyyy",
            shortTime:      "h:MM TT",
            mediumTime:     "h:MM:ss TT",
            longTime:       "h:MM:ss TT Z",
            isoDate:        "yyyy-mm-dd",
            isoTime:        "HH:MM:ss",
            isoDateTime:    "yyyy-mm-dd'T'HH:MM:ss",
            isoUtcDateTime: "UTC:yyyy-mm-dd'T'HH:MM:ss'Z'"
        };
        
        // Internationalization strings
        dateFormat.i18n = {
            dayNames: [
                "Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat",
                "Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"
            ],
            monthNames: [
                "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec",
                "January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"
            ]
        };
        
        // For convenience...
        Date.prototype.format = function (mask, utc) {
            return dateFormat(this, mask, utc);
        };

        /**********FIN DATE FORMAT*********/

        //tiempo.
        var tdTimers = $(".timercont");
        tdTimers.each(function(key, value){
            var id = $(value).attr('data-idorden');
            var url = $("#urltime_"+id).val();
            $.ajax({
              url: 'time',
              type: 'post',
              dataType: 'json',
              data: {id:id}
            })
            .done(function(result){
                setInterval(function(){
                    var da = result[0].created_at;
                    var ultimo = new Date(da.substr(0,4)+"/"+da.substr(5,2)+"/"+da.substr(8,2)+" "+da.substr(11,8));
                    var actual = new Date();
                    var diff=new Date(actual - ultimo);
                    var hora=0;
                    var minutos=0;
                    var segundos=0;
                    
                    if(diff.getUTCHours() <= 9){
                      hora = "0"+diff.getUTCHours();
                    }else{
                      hora = diff.getUTCHours();
                    }

                    if(diff.getUTCMinutes() <= 9){
                      minutos = "0"+diff.getUTCMinutes();
                    }else{
                      minutos = diff.getUTCMinutes();
                    }

                    if(diff.getUTCSeconds() <= 9){
                      segundos = "0"+diff.getUTCSeconds();
                    }else{
                      segundos = diff.getUTCSeconds();
                    }
                    $(value).attr("data-timer", hora+":"+minutos+":"+segundos);
                },1000)
            })
            .fail(function(result){
                console.log(result);
                $(value).attr("data-timer","Error con el conteo");
            });
        });

        /*Código para verificar tiempos de la orden*/
        setInterval(function(){
            notificar();
            tiempo_orden();
        },10000);

        setInterval(function(){
            var URLactual = window.location;
            
            if(URLactual == "http://carrier.pidafacil.com/delivery_pidafacil"){
                location.reload();
            }
        },60000);

        function tiempo_orden(){
            $.ajax({
                url: 'total_tiempo',
                type: 'post',
                dataType: 'json',
                data: {
                }
            })
            .done(function (result) {
                
                /*Ejemplo tiempo transcurrido*/
                $.each(result,function(index, value){
                    today = new Date();
                    var dateString = today.format("yyyy-mm-dd HH:MM:ss");
                    
                    var da = value['created_at'];
                    var orden = value['order_id'];
                    var ultimo = new Date(da.substr(0,4)+"/"+da.substr(5,2)+"/"+da.substr(8,2)+" "+da.substr(11,8));
                    var actual = new Date();
                    var diff=new Date(actual - ultimo);
                    var hora=0;
                    var minutos=0;
                    var segundos=0;

                    var estado = value['order_status_id'];
                    
                    if(diff.getUTCHours() <= 9){
                      hora = "0"+diff.getUTCHours();
                    }else{
                      hora = diff.getUTCHours();
                    }

                    if(diff.getUTCMinutes() <= 9){
                      minutos = "0"+diff.getUTCMinutes();
                    }else{
                      minutos = diff.getUTCMinutes();
                    }

                    if(diff.getUTCSeconds() <= 9){
                      segundos = "0"+diff.getUTCSeconds();
                    }else{
                      segundos = diff.getUTCSeconds();
                    }

                    if(minutos >= 15 && minutos <= 20 && estado < 4){
                        $("#col_"+orden).css("background-color","#E8E112");
                    }else if(hora > 00 && estado < 4){
                        $("#col_"+orden).css("background-color","#D02A2A");
                    }else if(minutos > 20 && estado < 4){
                        $("#col_"+orden).css("background-color","#D02A2A");
                    }else if(hora == 00 && minutos < 50 && estado == 4){
                        $("#col_"+orden).css("background-color","white");
                    }else if(hora == 00 && minutos >= 50 && minutos <= 60 && estado == 4){
                        $("#col_"+orden).css("background-color","#E8E112");
                    }else if(hora == 00 && minutos > 60 && estado == 4){
                        $("#col_"+orden).css("background-color","#D02A2A");
                    }else if(hora > 00 && estado == 4){
                        $("#col_"+orden).css("background-color","#D02A2A");
                    }
                });
            })
            .fail(function (result){
                
            });
        }

        /*Notificaciones nueva orden*/
        var notif = "";
        var allN = new Array();//arreglo que se llena con los codigos de ordenes actuales
        function notificar(){
            $.ajax({
                url: 'nuevas_ordenes_visor',
                type: 'post',
                dataType: 'json',
                data: {
                }
            })
            .done(function (result) {
                var text = "";
                //recorro todas las ordenes actuales
                $.each(result,function(index, value){
                    var estado = value['order_status_id'];
                    if(estado > 5){
                        if(allN.length==0){//si esta en 0 significa que es la primera vez que se recorre
                            //$('#sound_tag')[0].play();
                            var options = {
                                body: "Código de la orden: "+value['order_cod']+ '     '+"Restaurante: "+value['name'],
                                icon: "data:image/png;base64,"
                            };
                            
                            notif = new Notification("Nueva orden", options);
                            notif.onclick = function(){
                                //location.href='http://{{ $_SERVER['HTTP_HOST']."/pidafacil-delivery/public/delivery_pidafacil?fillter=1"}}';
                                location.href='http://localhost/pf-delivery/public/delivery_pidafacil';
                                //$('#sound_tag')[0].prop("currentTime",0);//detengo el sonido de la alerta
                            }
                            setTimeout(function(){
                                notif.close();
                            }, 8000);
                        }else{//si entra aqui es porque ya se recorrió la primera vez y no mostrará ordenes repetidas
                            if(jQuery.inArray(value['order_cod'], allN) == -1) {//evalua si el codigo ya esta en el array
                                //$('#sound_tag')[0].play();
                                var options = {
                                    body: "Código de la orden: "+value['order_cod']+ '     '+"Restaurante: "+value['name'],
                                    icon: "data:image/png;base64,"
                                };

                                notif = new Notification("Nueva orden", options);
                                notif.onclick = function(){
                                    //location.href='http://{{ $_SERVER['HTTP_HOST']."/pidafacil-delivery/public/delivery_pidafacil?fillter=1"}}';
                                    location.href='http://localhost/pf-delivery/public/delivery_pidafacil';
                                    //$('#sound_tag')[0].prop("currentTime",0);
                                }
                                setTimeout(function(){
                                    notif.close();
                                }, 8000);
                            }else{
                                console.log('esta en el array');
                            }
                        }
                        if(index==0){//le concateno a la variable text todos los codigos de las ordenes
                            text += value['order_cod'];
                        }else{
                            text += ","+value['order_cod'];
                        }    
                    }
                });
                allN = text.split(",");//le asigno los codigos de ordenes al arreglo
                console.log(allN);
            })
            .fail(function (result){
                $btn.button('reset');
                alert("No se pudo realizar la acción, por favor intenta de nuevo");
            });
        }

        /*SECCIÓN PARA GENERAR REPORTES*/

        $("#motivo_busqueda").change(function(){
            var valor = $('#motivo_busqueda').val();
            $("#motivo2").val(valor);

            if(valor == "fecha"){
                $("#mostrar_fechas").show();
                $("#moto_list").hide();
                $("#pago_list").hide();
                $("#mes_list").hide();
                $("#selectRes").hide();
            }else if(valor == "moto"){
                $("#mostrar_fechas").hide();
                $("#moto_list").show();
                $("#pago_list").hide();
                $("#mes_list").hide();
                $("#selectRes").hide();
            }else if(valor == "pago"){
                $("#pago_list").show();
                $("#mostrar_fechas").hide();
                $("#moto_list").hide();
                $("#mes_list").hide();
                $("#selectRes").hide();
            }else if(valor == 'restaurant'){
                $("#selectRes").show();
                $("#mostrar_fechas").hide();
                $("#mes_list").hide();
            }else if(valor == 'mes'){
                $("#selectRes").hide();
                $("#mostrar_fechas").hide();
                $("#mes_list").show();
                $("#selectRes").show();
            }
        });

        /*$('#datepicker1').datepicker({
            dateFormat: 'yy-mm-dd'
        });

        $('#datepicker2').datepicker({
            dateFormat: 'yy-mm-dd'
        });*/

        $("#buscar").on('click', function(){
            var motivo = $("#motivo_busqueda option:selected").val();
            
            if(motivo == "fecha"){
                var fecha1 = $("#datepicker1").val();
                var fecha2 = $("#datepicker2").val();

                var caracter = "T";
                var fecha_ini = fecha1.replace(caracter,' ');
                var fecha_f = fecha2.replace(caracter,' ');
                var sumatoria = 0;

                $.ajax({
                    url: 'busqueda_reporte',
                    type: 'post',
                    dataType: 'json',
                    data: {
                       motivo : motivo,
                       fecha_inicio : fecha_ini,
                       fecha_fin : fecha_f  
                    }
                })
                .done(function ( result ){
                    console.log(result);
                    
                    if(result.length == ""){
                        $("#tablaReporte").append("<tr><td colspan='10' style='font-size:15pt; text-align:center;'>No hay resultados de pedidos</td></tr>");
                    }else{
                        $.each(result, function(index, value){
                            console.log("index"+index);
                            /* Vamos agregando a nuestra tabla las filas necesarias */
                            if(value.payment_method_id == 1){
                                var pago = 'Efectivo';
                            }else if(value.payment_method_id == 2){
                                var pago = 'Tarjeta';
                            }else if(value.payment_method_id == 3){
                                var pago = 'Tigo Money';
                            }

                            if(value.order_status_id == 1){
                                var status = "Pendiente";
                            }else if(value.order_status_id == 2){
                                var status = "Cobrada";
                            }else if(value.order_status_id == 3){
                                var status = "Aceptada";
                            }else if(value.order_status_id == 4){
                                var status = "Despachada";
                            }else if(value.order_status_id == 5){
                                var status = "Completada";
                            }else if(value.order_status_id == 6){
                                var status = "Cancelada";
                            }else if(value.order_status_id == 7){
                                var status = "Cancelada";
                            }else if(value.order_status_id == 8){
                                var status = "Incobrable";
                            }else if(value.order_status_id == 9){
                                var status = "Cancelada";
                            }else if(value.order_status_id == 12){
                                var status = "Pendiente";
                            }else if(value.order_status_id == 13){
                                var status = "Sin Estado";
                            }else if(value.order_status_id == 'Pendiente'){
                                var status = "Sin Estado";
                            }

                            $("#tablaReporte").append("<tr><td>" + value.order_cod + "</td><td>" + value.created_at + "</td><td>" + pago + "</td><td>" + value.address + "</td><td>" + value.zone + "</td><td>$" + value.cobro_motoboy + "</td><td>" + value.name + "</td><td>" + value.nombre + "</td><td>" + status + "</td><td><button id='order_id_"+ value.order_id +"' class='btn btn-primary test'>Ver detalle</button></td></tr>");
                            
                            $("#order_id_"+value.order_id).on('click', function(){
                                
                                $("#detalleOrden").modal("show");
                                /*Datos del cliente en la orden*/
                                $("#cod_orden").text(value.order_cod);
                                //$("#ord_name").text(value.customer);
                                //$("#ord_address").text(value.address);
                                
                                /*if(value.reference != ''){
                                    $("#ord_ref").text(value.reference);
                                }else{
                                    $("#ord_ref").text('Sin Referencia');
                                }*/

                                if(value.zone != ''){
                                    $("#ord_zone").text(value.zone);
                                }else{
                                    $("#ord_zone").text('Sin Zona');
                                }
                                
                                //Tiempo total de la orden
                                var diferencia = Math.abs(value.tiempoC - value.created_at);
                                var da = value.created_at;
                                var da2 = value.tiempoC;
                                var ultimo = new Date(da.substr(0,4)+"/"+da.substr(5,2)+"/"+da.substr(8,2)+" "+da.substr(11,8));
                                var actual = new Date(da2.substr(0,4)+"/"+da2.substr(5,2)+"/"+da2.substr(8,2)+" "+da2.substr(11,8));
                                var diff=new Date(actual - ultimo);
                                var hora=0;
                                var minutos=0;
                                var segundos=0;
                                if(diff.getUTCHours() <= 9){
                                  hora = "0"+diff.getUTCHours();
                                }else{
                                  hora = diff.getUTCHours();
                                }

                                if(diff.getUTCMinutes() <= 9){
                                  minutos = "0"+diff.getUTCMinutes();
                                }else{
                                  minutos = diff.getUTCMinutes();
                                }

                                if(diff.getUTCSeconds() <= 9){
                                  segundos = "0"+diff.getUTCSeconds();
                                }else{
                                  segundos = diff.getUTCSeconds();
                                }

                                $("#tot_tie").text(hora+":"+minutos+":"+segundos);

                                $("#ord_date").text(value.created_at);
                                $("#ord_total").text('$ '+value.order_total);
                                //$("#tot_tie").text(diferencia);

                                /*Datos de pago*/
                                var com = value.commission_percentage;
                                var comision = ((value.order_total * com) / 100).toFixed(2);
                                var pago_restaurante = (value.order_total - comision).toFixed(2);
                                var total = parseFloat(value.order_total);
                                var envio = parseFloat(value.shipping_charge);
                                var pago_c = (total + envio).toFixed(2);
                                var pago_cliente = Math.round(value.order_total + value.shipping_charge);

                                $("#pago_rest").text(pago_restaurante);
                                $("#costo_envio").text(value.shipping_charge);
                                $("#total_cust").text(pago_c);
                                $("#cambio").text(value.pay_change);
                            });
                            sumatoria = (sumatoria + parseFloat(value.cobro_motoboy));
                        });
                        $("#tablaReporte").append("<tr><td colspan='2' style='text-align:right;'>Total</td><td></td><td></td><td></td><td>$ "+sumatoria.toFixed(2)+"</td><td></td><td></td><td></td><td></td></tr>");
                    }
                })
                .fail(function (result){
                  alert("Error al consultar los datos");
                });

            }else if(motivo == "moto"){
                var motorista = $("#moto option:selected").val();
                var sumatoria = 0;

                $.ajax({
                    url: 'busqueda_reporte',
                    type: 'post',
                    dataType: 'json',
                    data: {
                       motivo : motivo,
                       motorista : motorista  
                    }
                })
                .done(function ( result ){
                    
                    if(result.length == ""){
                        $("#tablaReporte").append("<tr><td colspan='10' style='font-size:15pt; text-align:center;'>No hay resultados de pedidos</td></tr>");
                    }else{
                        $.each(result, function(index, value){
                            /* Vamos agregando a nuestra tabla las filas necesarias */
                            
                            if(value.payment_method_id == 1){
                                var pago = 'Efectivo';
                            }else if(value.payment_method_id == 2){
                                var pago = 'Tarjeta';
                            }else if(value.payment_method_id == 3){
                                var pago = 'Tigo Money';
                            }

                            if(value.order_status_id == 1){
                                var status = "Pendiente";
                            }else if(value.order_status_id == 2){
                                var status = "Cobrada";
                            }else if(value.order_status_id == 3){
                                var status = "Aceptada";
                            }else if(value.order_status_id == 4){
                                var status = "Despachada";
                            }else if(value.order_status_id == 5){
                                var status = "Completada";
                            }else if(value.order_status_id == 6){
                                var status = "Cancelada";
                            }else if(value.order_status_id == 7){
                                var status = "Cancelada";
                            }else if(value.order_status_id == 8){
                                var status = "Incobrable";
                            }else if(value.order_status_id == 9){
                                var status = "Cancelada";
                            }else if(value.order_status_id == 12){
                                var status = "Completada";
                            }else if(value.order_status_id == 13){
                                var status = "Sin Estado";
                            }

                            $("#tablaReporte").append("<tr><td>" + value.order_cod + "</td><td>" + value.created_at + "</td><td>" + pago + "</td><td>" + value.address + "</td><td>" + value.zone + "</td><td>$" + value.cobro_motoboy + "</td><td>" + value.name + "</td><td>" + value.nombre + "</td><td>" + status + "</td><td><button id='order_id_"+ value.order_id +"' class='btn btn-primary test'>Ver detalle</button></td></tr>");

                            $("#order_id_"+value.order_id).on('click', function(){
                                
                                $("#detalleOrden").modal("show");
                                /*Datos del cliente en la orden*/
                                $("#cod_orden").text(value.order_cod);
                                //$("#ord_name").text(value.customer);
                                //$("#ord_address").text(value.address);
                                
                                /*if(value.reference != ''){
                                    $("#ord_ref").text(value.reference);
                                }else{
                                    $("#ord_ref").text('Sin Referencia');
                                }*/

                                if(value.zone != ''){
                                    $("#ord_zone").text(value.zone);
                                }else{
                                    $("#ord_zone").text('Sin Zona');
                                }

                                //Tiempo total de la orden
                                var diferencia = Math.abs(value.tiempoC - value.created_at);
                                var da = value.created_at;
                                var da2 = value.tiempoC;
                                var ultimo = new Date(da.substr(0,4)+"/"+da.substr(5,2)+"/"+da.substr(8,2)+" "+da.substr(11,8));
                                var actual = new Date(da2.substr(0,4)+"/"+da2.substr(5,2)+"/"+da2.substr(8,2)+" "+da2.substr(11,8));
                                var diff=new Date(actual - ultimo);
                                var hora=0;
                                var minutos=0;
                                var segundos=0;
                                if(diff.getUTCHours() <= 9){
                                  hora = "0"+diff.getUTCHours();
                                }else{
                                  hora = diff.getUTCHours();
                                }

                                if(diff.getUTCMinutes() <= 9){
                                  minutos = "0"+diff.getUTCMinutes();
                                }else{
                                  minutos = diff.getUTCMinutes();
                                }

                                if(diff.getUTCSeconds() <= 9){
                                  segundos = "0"+diff.getUTCSeconds();
                                }else{
                                  segundos = diff.getUTCSeconds();
                                }

                                $("#tot_tie").text(hora+":"+minutos+":"+segundos);

                                $("#ord_date").text(value.created_at);
                                $("#ord_total").text('$ '+value.order_total);

                                /*Datos de pago*/
                                var com = value.commission_percentage;
                                var comision = ((value.order_total * com) / 100).toFixed(2);
                                var pago_restaurante = (value.order_total - comision).toFixed(2);
                                var total = parseFloat(value.order_total);
                                var envio = parseFloat(value.shipping_charge);
                                var pago_c = (total + envio).toFixed(2);
                                var pago_cliente = Math.round(value.order_total + value.shipping_charge);

                                $("#pago_rest").text(pago_restaurante);
                                $("#costo_envio").text(value.shipping_charge);
                                $("#total_cust").text(pago_c);
                                $("#cambio").text(value.pay_change);
                            });
                            sumatoria = (sumatoria + parseFloat(value.cobro_motoboy));
                        });
                        $("#tablaReporte").append("<tr><td colspan='2' style='text-align:right;'>Total</td><td></td><td></td><td></td><td>$ "+sumatoria.toFixed(2)+"</td><td></td><td></td><td></td><td></td></tr>");
                    }
                })
                .fail(function (result){
                    alert("Error al consultar los datos");
                });

            }else if(motivo == "pago"){
                var pago = $("#tipo_pago option:selected").val();
                var sumatoria = 0;

                $.ajax({
                    url: 'busqueda_reporte',
                    type: 'post',
                    dataType: 'json',
                    data: {
                       motivo : motivo,
                       pago : pago
                    }
                })
                .done(function (result){

                    if(result.length == ""){
                        $("#tablaReporte").append("<tr><td colspan='10' style='font-size:15pt; text-align:center;'>No hay resultados de pedidos</td></tr>");
                    }else{
                        $.each(result, function(index, value){
                            /* Vamos agregando a nuestra tabla las filas necesarias */
                            if(value.payment_method_id == 1){
                                var pago = 'Efectivo';
                            }else if(value.payment_method_id == 2){
                                var pago = 'Tarjeta';
                            }else if(value.payment_method_id == 3){
                                var pago = 'Tigo Money';
                            }

                            if(value.order_status_id == 1){
                                var status = "Pendiente";
                            }else if(value.order_status_id == 2){
                                var status = "Cobrada";
                            }else if(value.order_status_id == 3){
                                var status = "Aceptada";
                            }else if(value.order_status_id == 4){
                                var status = "Despachada";
                            }else if(value.order_status_id == 5){
                                var status = "Completada";
                            }else if(value.order_status_id == 6){
                                var status = "Cancelada";
                            }else if(value.order_status_id == 7){
                                var status = "Cancelada";
                            }else if(value.order_status_id == 8){
                                var status = "Incobrable";
                            }else if(value.order_status_id == 9){
                                var status = "Cancelada";
                            }else if(value.order_status_id == 12){
                                var status = "Completada";
                            }else if(value.order_status_id == 13){
                                var status = "Sin Estado";
                            }
                            
                            $("#tablaReporte").append("<tr><td>" + value.order_cod + "</td><td>" + value.created_at + "</td><td>" + pago + "</td><td>" + value.address + "</td><td>" + value.zone + "</td><td>$" + value.cobro_motoboy + "</td><td>" + value.name + "</td><td>" + value.nombre + "</td><td>" + status + "</td><td><button id='order_id_"+ value.order_id +"' class='btn btn-primary test'>Ver detalle</button></td></tr>");

                            $("#order_id_"+value.order_id).on('click', function(){
                                
                                $("#detalleOrden").modal("show");
                                /*Datos del cliente en la orden*/
                                $("#cod_orden").text(value.order_cod);
                                //$("#ord_name").text(value.customer);
                                //$("#ord_address").text(value.address);
                                
                                /*if(value.reference != ''){
                                    $("#ord_ref").text(value.reference);
                                }else{
                                    $("#ord_ref").text('Sin Referencia');
                                }*/

                                if(value.zone != ''){
                                    $("#ord_zone").text(value.zone);
                                }else{
                                    $("#ord_zone").text('Sin Zona');
                                }
                                
                                //Tiempo total de la orden
                                var diferencia = Math.abs(value.tiempoC - value.created_at);
                                var da = value.created_at;
                                var da2 = value.tiempoC;
                                var ultimo = new Date(da.substr(0,4)+"/"+da.substr(5,2)+"/"+da.substr(8,2)+" "+da.substr(11,8));
                                var actual = new Date(da2.substr(0,4)+"/"+da2.substr(5,2)+"/"+da2.substr(8,2)+" "+da2.substr(11,8));
                                var diff=new Date(actual - ultimo);
                                var hora=0;
                                var minutos=0;
                                var segundos=0;
                                if(diff.getUTCHours() <= 9){
                                  hora = "0"+diff.getUTCHours();
                                }else{
                                  hora = diff.getUTCHours();
                                }

                                if(diff.getUTCMinutes() <= 9){
                                  minutos = "0"+diff.getUTCMinutes();
                                }else{
                                  minutos = diff.getUTCMinutes();
                                }

                                if(diff.getUTCSeconds() <= 9){
                                  segundos = "0"+diff.getUTCSeconds();
                                }else{
                                  segundos = diff.getUTCSeconds();
                                }

                                $("#tot_tie").text(hora+":"+minutos+":"+segundos);
                                
                                $("#ord_date").text(value.created_at);
                                $("#ord_total").text('$ '+value.order_total);

                                /*Datos de pago*/
                                var com = value.commission_percentage;
                                var comision = ((value.order_total * com) / 100).toFixed(2);
                                var pago_restaurante = (value.order_total - comision).toFixed(2);
                                var total = parseFloat(value.order_total);
                                var envio = parseFloat(value.shipping_charge);
                                var pago_c = (total + envio).toFixed(2);
                                var pago_cliente = Math.round(value.order_total + value.shipping_charge);

                                $("#pago_rest").text(pago_restaurante);
                                $("#costo_envio").text(value.shipping_charge);
                                $("#total_cust").text(pago_c);
                                $("#cambio").text(value.pay_change);
                            });
                            
                            sumatoria = (sumatoria + parseFloat(value.cobro_motoboy));
                        });
                        $("#tablaReporte").append("<tr><td colspan='2' style='text-align:right;'>Total</td><td></td><td></td><td></td><td>$ "+sumatoria.toFixed(2)+"</td><td></td><td></td><td></td><td></td></tr>");
                    }
                })
                .fail(function (result){
                    alert("Error al consultar los datos");
                });
            }
        });

        /*FIN SECCIÓN PARA GENERAR REPORTES*/

        $("#limpiar").click(function(){
            $("#formularioExc").hide();
            location.reload();
        });

        /*MOSTRAR MODAL*/
        $(".mostrarDetalle").on('click', function(){
            var $btn = $(this);
            var id_elemento = $(this).attr('id');
            var id_pedido = id_elemento.split("_");
            
            $("#order_detail_"+id_pedido[2]).modal("show");
        });

        /*COMIENZA REPORTE DE VENTAS NEW 23 May*/

        $("#BVentaDetalle").on('click', function(){

            $("#formularioExc").show();

            if ($('#cuerpoT').length == 0){
            }else{
                $("#cuerpoT").empty();
            }
            
            var motivo = $("#motivo_busqueda option:selected").val();
            
            if(motivo == "fecha"){
                var fecha1 = $("#datepicker1").val();
                var fecha2 = $("#datepicker2").val();

                $("#motivo2").val(motivo);
                $("#fechaN1").val(fecha1);
                $("#fechaN2").val(fecha2);

                var caracter = "T";
                var fecha_ini = fecha1.replace(caracter,' ');
                var fecha_f = fecha2.replace(caracter,' ');
                var sumatoria = 0;
                var Ssubtotal = 0;
                var Senvio = 0;
                var Starjeta = 0;
                var Stotal = 0;
                

                $.ajax({
                    url: 'rep_vent',
                    type: 'post',
                    dataType: 'json',
                    data: {
                       motivo : motivo,
                       fecha_inicio : fecha_ini,
                       fecha_fin : fecha_f  
                    }
                })
                .done(function ( result ){
                    console.log(result);
                    
                    if(result.length == ""){
                        $("#tablaReporte>tbody").append("<tr><td colspan='10' style='font-size:15pt; text-align:center;'>No hay resultados de pedidos</td></tr>");
                    }else{
                        $.each(result, function(index, value){
                            console.log("index"+index);
                            
                            $("#tablaReporte>tbody").append("<tr><td>" + value.orden + "</td><td>" + value.fecha + "</td><td>" + value.nombre_cliente +' '+ value.apellido_cliente + "</td><td>" + value.email + "</td><td>" + value.zona + "</td><td>" + value.restaurante + "</td><td>" + value.estado + "</td><td>" + value.motociclista + "</td><td>" + value.tipo_pago + "</td><td>$" + value.subtotal + "</td><td>$" + value.costo_envio + "</td><td>$" + value.cargo_tarjeta + "</td><td>$" + value.total + "</td></tr>");
                            
                            $("#order_id_"+value.order_id).on('click', function(){
                                
                                $("#detalleOrden").modal("show");
                                /*Datos del cliente en la orden*/
                                $("#cod_orden").text(value.order_cod);
                            
                                if(value.zone != ''){
                                    $("#ord_zone").text(value.zone);
                                }else{
                                    $("#ord_zone").text('Sin Zona');
                                }
                                
                                //Tiempo total de la orden
                                var diferencia = Math.abs(value.tiempoC - value.created_at);
                                var da = value.created_at;
                                var da2 = value.tiempoC;
                                var ultimo = new Date(da.substr(0,4)+"/"+da.substr(5,2)+"/"+da.substr(8,2)+" "+da.substr(11,8));
                                var actual = new Date(da2.substr(0,4)+"/"+da2.substr(5,2)+"/"+da2.substr(8,2)+" "+da2.substr(11,8));
                                var diff=new Date(actual - ultimo);
                                var hora=0;
                                var minutos=0;
                                var segundos=0;

                                if(diff.getUTCHours() <= 9){
                                  hora = "0"+diff.getUTCHours();
                                }else{
                                  hora = diff.getUTCHours();
                                }

                                if(diff.getUTCMinutes() <= 9){
                                  minutos = "0"+diff.getUTCMinutes();
                                }else{
                                  minutos = diff.getUTCMinutes();
                                }

                                if(diff.getUTCSeconds() <= 9){
                                  segundos = "0"+diff.getUTCSeconds();
                                }else{
                                  segundos = diff.getUTCSeconds();
                                }

                                $("#tot_tie").text(hora+":"+minutos+":"+segundos);

                                $("#ord_date").text(value.created_at);
                                $("#ord_total").text('$ '+value.order_total);
                                //$("#tot_tie").text(diferencia);

                                /*Datos de pago*/
                                var com = value.commission_percentage;
                                var comision = ((value.order_total * com) / 100).toFixed(2);
                                var pago_restaurante = (value.order_total - comision).toFixed(2);
                                var total = parseFloat(value.order_total);
                                var envio = parseFloat(value.shipping_charge);
                                var pago_c = (total + envio).toFixed(2);
                                var pago_cliente = Math.round(value.order_total + value.shipping_charge);

                                $("#pago_rest").text(pago_restaurante);
                                $("#costo_envio").text(value.shipping_charge);
                                $("#total_cust").text(pago_c);
                                $("#cambio").text(value.pay_change);
                            });

                            Ssubtotal = (Ssubtotal + parseFloat(value.subtotal));
                            Senvio = (Senvio + parseFloat(value.costo_envio));
                            Starjeta = (Starjeta + parseFloat(value.cargo_tarjeta));
                            Stotal = (Stotal + parseFloat(value.total));
                        });                        
                        $("#tablaReporte>tbody").append("<tr><th>Total</th><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td>$" + Ssubtotal.toFixed(2) + "</td><td>$" + Senvio.toFixed(2) + "</td><td>$" + Starjeta.toFixed(2) + "</td><td>$" + Stotal.toFixed(2) + "</td></tr>");
                    }
                })
                .fail(function (result){
                  alert("Error al consultar los datos");
                });
            }else if(motivo == "moto"){
                var motorista = $("#moto option:selected").val();
                var sumatoria = 0;

                $.ajax({
                    url: 'rep_vent',
                    type: 'post',
                    dataType: 'json',
                    data: {
                       motivo : motivo,
                       motorista : motorista  
                    }
                })
                .done(function ( result ){
                    
                    if(result.length == ""){
                        $("#tablaReporte").append("<tr><td colspan='10' style='font-size:15pt; text-align:center;'>No hay resultados de pedidos</td></tr>");
                    }else{
                        $.each(result, function(index, value){
                            /* Vamos agregando a nuestra tabla las filas necesarias */
                            
                            $("#tablaReporte").append("<tr><td>" + value.orden + "</td><td>" + value.fecha + "</td><td>" + value.nombre_cliente +' '+ value.apellido_cliente + "</td><td>" + value.email + "</td><td>" + value.zona + "</td><td>" + value.restaurante + "</td><td>" + value.estado + "</td><td>" + value.motociclista + "</td><td>" + value.tipo_pago + "</td><td>$" + value.subtotal + "</td><td>$" + value.costo_envio + "</td><td>$" + value.cargo_tarjeta + "</td><td>$" + value.total + "</td></tr>");

                            $("#order_id_"+value.order_id).on('click', function(){
                                
                                $("#detalleOrden").modal("show");
                                /*Datos del cliente en la orden*/
                                $("#cod_orden").text(value.order_cod);
                        
                                if(value.zone != ''){
                                    $("#ord_zone").text(value.zone);
                                }else{
                                    $("#ord_zone").text('Sin Zona');
                                }

                                //Tiempo total de la orden
                                var diferencia = Math.abs(value.tiempoC - value.created_at);
                                var da = value.created_at;
                                var da2 = value.tiempoC;
                                var ultimo = new Date(da.substr(0,4)+"/"+da.substr(5,2)+"/"+da.substr(8,2)+" "+da.substr(11,8));
                                var actual = new Date(da2.substr(0,4)+"/"+da2.substr(5,2)+"/"+da2.substr(8,2)+" "+da2.substr(11,8));
                                var diff=new Date(actual - ultimo);
                                var hora=0;
                                var minutos=0;
                                var segundos=0;

                                if(diff.getUTCHours() <= 9){
                                  hora = "0"+diff.getUTCHours();
                                }else{
                                  hora = diff.getUTCHours();
                                }

                                if(diff.getUTCMinutes() <= 9){
                                  minutos = "0"+diff.getUTCMinutes();
                                }else{
                                  minutos = diff.getUTCMinutes();
                                }

                                if(diff.getUTCSeconds() <= 9){
                                  segundos = "0"+diff.getUTCSeconds();
                                }else{
                                  segundos = diff.getUTCSeconds();
                                }

                                $("#tot_tie").text(hora+":"+minutos+":"+segundos);

                                $("#ord_date").text(value.created_at);
                                $("#ord_total").text('$ '+value.order_total);

                                /*Datos de pago*/
                                var com = value.commission_percentage;
                                var comision = ((value.order_total * com) / 100).toFixed(2);
                                var pago_restaurante = (value.order_total - comision).toFixed(2);
                                var total = parseFloat(value.order_total);
                                var envio = parseFloat(value.shipping_charge);
                                var pago_c = (total + envio).toFixed(2);
                                var pago_cliente = Math.round(value.order_total + value.shipping_charge);

                                $("#pago_rest").text(pago_restaurante);
                                $("#costo_envio").text(value.shipping_charge);
                                $("#total_cust").text(pago_c);
                                $("#cambio").text(value.pay_change);
                            });
                            sumatoria = (sumatoria + parseFloat(value.cobro_motoboy));
                        });
                        $("#tablaReporte").append("<tr><td colspan='2' style='text-align:right;'>Total</td><td></td><td></td><td></td><td>$ "+sumatoria.toFixed(2)+"</td><td></td><td></td><td></td><td></td></tr>");
                    }
                })
                .fail(function (result){
                    alert("Error al consultar los datos");
                });
            }else if(motivo == "pago"){
                var pago = $("#tipo_pago option:selected").val();
                var sumatoria = 0;

                $.ajax({
                    url: 'rep_vent',
                    type: 'post',
                    dataType: 'json',
                    data: {
                       motivo : motivo,
                       pago : pago
                    }
                })
                .done(function (result){

                    if(result.length == ""){
                        $("#tablaReporte").append("<tr><td colspan='10' style='font-size:15pt; text-align:center;'>No hay resultados de pedidos</td></tr>");
                    }else{
                        $.each(result, function(index, value){
                           
                            $("#tablaReporte").append("<tr><td>" + value.orden + "</td><td>" + value.fecha + "</td><td>" + value.nombre_cliente +' '+ value.apellido_cliente + "</td><td>" + value.email + "</td><td>" + value.zona + "</td><td>" + value.restaurante + "</td><td>" + value.estado + "</td><td>" + value.motociclista + "</td><td>" + value.tipo_pago + "</td><td>$" + value.subtotal + "</td><td>$" + value.costo_envio + "</td><td>$" + value.cargo_tarjeta + "</td><td>$" + value.total + "</td></td></tr>");

                            $("#order_id_"+value.order_id).on('click', function(){
                                
                                $("#detalleOrden").modal("show");
                                /*Datos del cliente en la orden*/
                                $("#cod_orden").text(value.order_cod);
                            
                                if(value.zone != ''){
                                    $("#ord_zone").text(value.zone);
                                }else{
                                    $("#ord_zone").text('Sin Zona');
                                }
                                
                                //Tiempo total de la orden
                                var diferencia = Math.abs(value.tiempoC - value.created_at);
                                var da = value.created_at;
                                var da2 = value.tiempoC;
                                var ultimo = new Date(da.substr(0,4)+"/"+da.substr(5,2)+"/"+da.substr(8,2)+" "+da.substr(11,8));
                                var actual = new Date(da2.substr(0,4)+"/"+da2.substr(5,2)+"/"+da2.substr(8,2)+" "+da2.substr(11,8));
                                var diff=new Date(actual - ultimo);
                                var hora=0;
                                var minutos=0;
                                var segundos=0;

                                if(diff.getUTCHours() <= 9){
                                  hora = "0"+diff.getUTCHours();
                                }else{
                                  hora = diff.getUTCHours();
                                }

                                if(diff.getUTCMinutes() <= 9){
                                  minutos = "0"+diff.getUTCMinutes();
                                }else{
                                  minutos = diff.getUTCMinutes();
                                }

                                if(diff.getUTCSeconds() <= 9){
                                  segundos = "0"+diff.getUTCSeconds();
                                }else{
                                  segundos = diff.getUTCSeconds();
                                }

                                $("#tot_tie").text(hora+":"+minutos+":"+segundos);
                                
                                $("#ord_date").text(value.created_at);
                                $("#ord_total").text('$ '+value.order_total);

                                /*Datos de pago*/
                                var com = value.commission_percentage;
                                var comision = ((value.order_total * com) / 100).toFixed(2);
                                var pago_restaurante = (value.order_total - comision).toFixed(2);
                                var total = parseFloat(value.order_total);
                                var envio = parseFloat(value.shipping_charge);
                                var pago_c = (total + envio).toFixed(2);
                                var pago_cliente = Math.round(value.order_total + value.shipping_charge);

                                $("#pago_rest").text(pago_restaurante);
                                $("#costo_envio").text(value.shipping_charge);
                                $("#total_cust").text(pago_c);
                                $("#cambio").text(value.pay_change);
                            });
                            
                            sumatoria = (sumatoria + parseFloat(value.cobro_motoboy));
                        });
                        $("#tablaReporte").append("<tr><td colspan='2' style='text-align:right;'>Total</td><td></td><td></td><td></td><td>$ "+sumatoria.toFixed(2)+"</td><td></td><td></td><td></td><td></td></tr>");
                    }
                })
                .fail(function (result){
                    alert("Error al consultar los datos");
                });
            }
        });

        //codigo para exportar los datos a excel
        $("#exportar").on('click', function(){
            
            var motivo = $("#motivo_busqueda option:selected").val();
            
            if(motivo == "fecha"){
                var fecha1 = $("#datepicker1").val();
                var fecha2 = $("#datepicker2").val();

                var caracter = "T";
                var fecha_ini = fecha1.replace(caracter,' ');
                var fecha_f = fecha2.replace(caracter,' ');
                var sumatoria = 0;

                $.ajax({
                    url: 'rep_vent',
                    type: 'post',
                    dataType: 'json',
                    data: {
                       motivo : motivo,
                       fecha_inicio : fecha_ini,
                       fecha_fin : fecha_f  
                    }
                })
                .done(function ( result ){
                    console.log(result);
                })
                .fail(function (result){
                  alert("Error al consultar los datos");
                });

            }else if(motivo == "moto"){
                var motorista = $("#moto option:selected").val();
                var sumatoria = 0;

                $.ajax({
                    url: 'rep_vent',
                    type: 'post',
                    dataType: 'json',
                    data: {
                       motivo : motivo,
                       motorista : motorista  
                    }
                })
                .done(function ( result ){
                    
                })
                .fail(function (result){
                    alert("Error al consultar los datos");
                });

            }else if(motivo == "pago"){
                var pago = $("#tipo_pago option:selected").val();
                var sumatoria = 0;

                $.ajax({
                    url: 'rep_vent',
                    type: 'post',
                    dataType: 'json',
                    data: {
                       motivo : motivo,
                       pago : pago
                    }
                })
                .done(function (result){

                })
                .fail(function (result){
                    alert("Error al consultar los datos");
                });
            }
        });

        $("#generarReporteV").on('click', function(){
            $("#formularioExc").show();

            if($('#cuerpoT').length == 0){
            }else{
                $("#cuerpoT").empty();
            }            
            
            var motivo = $("#motivo_busqueda option:selected").val();
            
            if(motivo == "fecha"){
                var fecha1 = $("#datepicker1").val();
                var fecha2 = $("#datepicker2").val();

                $("#motivo2").val(motivo);
                $("#fechaN1").val(fecha1);
                $("#fechaN2").val(fecha2);

                var caracter = "T";
                var fecha_ini = fecha1.replace(caracter,' ');
                var fecha_f = fecha2.replace(caracter,' ');
                var sumPR = 0;
                var sumE = 0;
                var sumCR = 0;
                var sumCE = 0;
                var sumTC = 0;
                var sumTV = 0;
                var totalC = 0;
                var sumNP = 0;

                $.ajax({
                    url: 'ventaRep',
                    type: 'post',
                    dataType: 'json',
                    data: {
                       motivo : motivo,
                       fecha_inicio : fecha_ini,
                       fecha_fin : fecha_f  
                    }
                })
                .done(function ( result ){
                    console.log(result.entregadas);
                    
                    if(result.ordenes.length == ""){
                        $("#tablaReporte>tbody").append("<tr><td colspan='10' style='font-size:15pt; text-align:center;'>No hay resultados de pedidos</td></tr>");
                    }else{
                        $.each(result.ordenes, function(index, value){
                            console.log("index"+index);
                            totalC = parseFloat(value.comision_restaurante) + parseFloat(value.comision_envio);
                            
                            $("#tablaReporte>tbody").append("<tr><td>" + value.fecha + "</td><td>$" + value.pago_restaurante + "</td><td>$" + value.envio + "</td><td>$" + value.comision_restaurante + "</td><td>$" + value.comision_envio + "</td><td>$" + totalC.toFixed(2) + "</td><td>$" + value.total + "</td><td>" + value.estado + "</td></tr>");
                            
                            $("#order_id_"+value.order_id).on('click', function(){
                                
                                $("#detalleOrden").modal("show");
                                /*Datos del cliente en la orden*/
                                $("#cod_orden").text(value.order_cod);
                            
                                if(value.zone != ''){
                                    $("#ord_zone").text(value.zone);
                                }else{
                                    $("#ord_zone").text('Sin Zona');
                                }
                                
                                //Tiempo total de la orden
                                var diferencia = Math.abs(value.tiempoC - value.created_at);
                                var da = value.created_at;
                                var da2 = value.tiempoC;
                                var ultimo = new Date(da.substr(0,4)+"/"+da.substr(5,2)+"/"+da.substr(8,2)+" "+da.substr(11,8));
                                var actual = new Date(da2.substr(0,4)+"/"+da2.substr(5,2)+"/"+da2.substr(8,2)+" "+da2.substr(11,8));
                                var diff=new Date(actual - ultimo);
                                var hora=0;
                                var minutos=0;
                                var segundos=0;

                                if(diff.getUTCHours() <= 9){
                                  hora = "0"+diff.getUTCHours();
                                }else{
                                  hora = diff.getUTCHours();
                                }

                                if(diff.getUTCMinutes() <= 9){
                                  minutos = "0"+diff.getUTCMinutes();
                                }else{
                                  minutos = diff.getUTCMinutes();
                                }

                                if(diff.getUTCSeconds() <= 9){
                                  segundos = "0"+diff.getUTCSeconds();
                                }else{
                                  segundos = diff.getUTCSeconds();
                                }

                                $("#tot_tie").text(hora+":"+minutos+":"+segundos);

                                $("#ord_date").text(value.created_at);
                                $("#ord_total").text('$ '+value.order_total);
                                //$("#tot_tie").text(diferencia);

                                /*Datos de pago*/
                                var com = value.commission_percentage;
                                var comision = ((value.order_total * com) / 100).toFixed(2);
                                var pago_restaurante = (value.order_total - comision).toFixed(2);
                                var total = parseFloat(value.order_total);
                                var envio = parseFloat(value.shipping_charge);
                                var pago_c = (total + envio).toFixed(2);
                                var pago_cliente = Math.round(value.order_total + value.shipping_charge);

                                $("#pago_rest").text(pago_restaurante);
                                $("#costo_envio").text(value.shipping_charge);
                                $("#total_cust").text(pago_c);
                                $("#cambio").text(value.pay_change);
                            });
                            sumPR = (sumPR + parseFloat(value.pago_restaurante));
                            sumE = (sumE + parseFloat(value.envio));
                            sumCR = (sumCR + parseFloat(value.comision_restaurante));
                            sumCE = (sumCE + parseFloat(value.comision_envio));
                            sumTC = (sumTC + parseFloat(totalC));
                            sumTV = (sumTV + parseFloat(value.total));
                            sumNP = (sumNP + parseInt(value.estado));
                        });

                        $("#tablaReporte>tbody").append("<tr><th style='text-align:right;'>Total</th><td>$"+sumPR.toFixed(2)+"</td><td>$"+sumE.toFixed(2)+"</td><td>$"+sumCR.toFixed(2)+"</td><td>$"+sumCE.toFixed(2)+"</td><td>$ "+sumTC.toFixed(2)+"</td><td>$ "+sumTV.toFixed(2)+"</td><td>"+sumNP+"</td></tr>");
                    }

                    /*Count pedidos entregados
                    var countP = 0;
                    var totalPed = 0;
                    $.each(result.entregadas, function(index, value){
                        if(value.estado == 'Entregada'){
                            totalPed = (totalPed + parseInt(1));
                        }
                    });
                    $("#tablaReporte>tbody").append("<tr><th style='text-align:right;'>Pedidos entregados: </th><td colspan='6'>"+totalPed+"</td></tr>");
                    */
                })
                .fail(function (result){
                  alert("Error al consultar los datos");
                });
            }else if(motivo == "restaurant"){
                var rest = $("#restauranteSel option:selected").val();
                var sumatoria = 0;
                var sumPR = 0;
                var sumE = 0;
                var sumCR = 0;
                var sumCE = 0;
                var sumTC = 0;
                var sumTV = 0;
                var totalC = 0;

                $("#RestN").val(rest);

                $.ajax({
                    url: 'ventaRep',
                    type: 'post',
                    dataType: 'json',
                    data: {
                       motivo : motivo,
                       rest : rest  
                    }
                })
                .done(function ( result ){
                    console.log(result);
                    
                    if(result.ordenes.length == ""){
                        $("#tablaReporte>tbody").append("<tr><td colspan='10' style='font-size:15pt; text-align:center;'>No hay resultados de pedidos</td></tr>");
                    }else{
                        $.each(result.ordenes, function(index, value){
                            console.log("index"+index);
                            totalC = parseFloat(value.comision_restaurante) + parseFloat(value.comision_envio);                            
                            $("#tablaReporte>tbody").append("<tr><td>" + value.fecha + "</td><td>$" + value.pago_restaurante + "</td><td>$" + value.envio + "</td><td>$" + value.comision_restaurante + "</td><td>$" + value.comision_envio + "</td><td>$" + totalC.toFixed(2) + "</td><td>$" + value.total + "</td></tr>");
                            
                            $("#order_id_"+value.order_id).on('click', function(){
                                
                                $("#detalleOrden").modal("show");
                                /*Datos del cliente en la orden*/
                                $("#cod_orden").text(value.order_cod);
                            
                                if(value.zone != ''){
                                    $("#ord_zone").text(value.zone);
                                }else{
                                    $("#ord_zone").text('Sin Zona');
                                }
                                
                                //Tiempo total de la orden
                                var diferencia = Math.abs(value.tiempoC - value.created_at);
                                var da = value.created_at;
                                var da2 = value.tiempoC;
                                var ultimo = new Date(da.substr(0,4)+"/"+da.substr(5,2)+"/"+da.substr(8,2)+" "+da.substr(11,8));
                                var actual = new Date(da2.substr(0,4)+"/"+da2.substr(5,2)+"/"+da2.substr(8,2)+" "+da2.substr(11,8));
                                var diff=new Date(actual - ultimo);
                                var hora=0;
                                var minutos=0;
                                var segundos=0;

                                if(diff.getUTCHours() <= 9){
                                  hora = "0"+diff.getUTCHours();
                                }else{
                                  hora = diff.getUTCHours();
                                }

                                if(diff.getUTCMinutes() <= 9){
                                  minutos = "0"+diff.getUTCMinutes();
                                }else{
                                  minutos = diff.getUTCMinutes();
                                }

                                if(diff.getUTCSeconds() <= 9){
                                  segundos = "0"+diff.getUTCSeconds();
                                }else{
                                  segundos = diff.getUTCSeconds();
                                }

                                $("#tot_tie").text(hora+":"+minutos+":"+segundos);

                                $("#ord_date").text(value.created_at);
                                $("#ord_total").text('$ '+value.order_total);
                                //$("#tot_tie").text(diferencia);

                                /*Datos de pago*/
                                var com = value.commission_percentage;
                                var comision = ((value.order_total * com) / 100).toFixed(2);
                                var pago_restaurante = (value.order_total - comision).toFixed(2);
                                var total = parseFloat(value.order_total);
                                var envio = parseFloat(value.shipping_charge);
                                var pago_c = (total + envio).toFixed(2);
                                var pago_cliente = Math.round(value.order_total + value.shipping_charge);

                                $("#pago_rest").text(pago_restaurante);
                                $("#costo_envio").text(value.shipping_charge);
                                $("#total_cust").text(pago_c);
                                $("#cambio").text(value.pay_change);
                            });
                            sumPR = (sumPR + parseFloat(value.pago_restaurante));
                            sumE = (sumE + parseFloat(value.envio));
                            sumCR = (sumCR + parseFloat(value.comision_restaurante));
                            sumCE = (sumCE + parseFloat(value.comision_envio));
                            sumTC = (sumTC + parseFloat(totalC));
                            sumTV = (sumTV + parseFloat(value.total));
                        });

                        $("#tablaReporte>tbody").append("<tr><th style='text-align:right;'>Total</th><td>$"+sumPR.toFixed(2)+"</td><td>$"+sumE.toFixed(2)+"</td><td>$"+sumCR.toFixed(2)+"</td><td>$"+sumCE.toFixed(2)+"</td><td>$ "+sumTC.toFixed(2)+"</td><td>$ "+sumTV.toFixed(2)+"</td></tr>");
                    }
                })
                .fail(function (result){
                  alert("Error al consultar los datos");
                });
            }else if(motivo == "mes"){
                var mes = $("#meses option:selected").val();
                var anio = $("#anio option:selected").val();
                var rest = $("#restauranteSel option:selected").val();
                
                $("#textoRest").text('* '+ $("#restauranteSel option:selected").text());
                
                var sumatoria = 0;
                var sumPR = 0;
                var sumE = 0;
                var sumCR = 0;
                var sumCE = 0;
                var sumTC = 0;
                var sumTV = 0;
                var totalC = 0;
                var sumNP = 0;

                $("#mesN").val(mes);
                $("#AnioN").val(anio);
                $("#RestN").val(rest);

                console.log(mes +' '+anio +' '+ rest);

                $.ajax({
                    url: 'ventaRep',
                    type: 'post',
                    dataType: 'json',
                    data: {
                       motivo : motivo,
                       mes : mes,
                       anio : anio,
                       rest : rest
                    }
                })
                .done(function ( result ){
                    console.log(result);
                    
                    if(result.ordenes.length == ""){
                        $("#tablaReporte>tbody").append("<tr><td colspan='10' style='font-size:15pt; text-align:center;'>No hay resultados de pedidos</td></tr>");
                    }else{
                        $.each(result.ordenes, function(index, value){
                            console.log("index"+index);
                            totalC = parseFloat(value.comision_restaurante) + parseFloat(value.comision_envio);                            
                            $("#tablaReporte>tbody").append("<tr><td>" + value.fecha + "</td><td>$" + value.pago_restaurante + "</td><td>$" + value.envio + "</td><td>$" + value.comision_restaurante + "</td><td>$" + value.comision_envio + "</td><td>$" + totalC.toFixed(2) + "</td><td>$" + value.total + "</td><td>" + value.estado + "</td></tr>");
                            $("#order_id_"+value.order_id).on('click', function(){
                                
                                $("#detalleOrden").modal("show");
                                /*Datos del cliente en la orden*/
                                $("#cod_orden").text(value.order_cod);
                            
                                if(value.zone != ''){
                                    $("#ord_zone").text(value.zone);
                                }else{
                                    $("#ord_zone").text('Sin Zona');
                                }
                                
                                //Tiempo total de la orden
                                var diferencia = Math.abs(value.tiempoC - value.created_at);
                                var da = value.created_at;
                                var da2 = value.tiempoC;
                                var ultimo = new Date(da.substr(0,4)+"/"+da.substr(5,2)+"/"+da.substr(8,2)+" "+da.substr(11,8));
                                var actual = new Date(da2.substr(0,4)+"/"+da2.substr(5,2)+"/"+da2.substr(8,2)+" "+da2.substr(11,8));
                                var diff=new Date(actual - ultimo);
                                var hora=0;
                                var minutos=0;
                                var segundos=0;

                                if(diff.getUTCHours() <= 9){
                                  hora = "0"+diff.getUTCHours();
                                }else{
                                  hora = diff.getUTCHours();
                                }

                                if(diff.getUTCMinutes() <= 9){
                                  minutos = "0"+diff.getUTCMinutes();
                                }else{
                                  minutos = diff.getUTCMinutes();
                                }

                                if(diff.getUTCSeconds() <= 9){
                                  segundos = "0"+diff.getUTCSeconds();
                                }else{
                                  segundos = diff.getUTCSeconds();
                                }

                                $("#tot_tie").text(hora+":"+minutos+":"+segundos);

                                $("#ord_date").text(value.created_at);
                                $("#ord_total").text('$ '+value.order_total);
                                //$("#tot_tie").text(diferencia);

                                /*Datos de pago*/
                                var com = value.commission_percentage;
                                var comision = ((value.order_total * com) / 100).toFixed(2);
                                var pago_restaurante = (value.order_total - comision).toFixed(2);
                                var total = parseFloat(value.order_total);
                                var envio = parseFloat(value.shipping_charge);
                                var pago_c = (total + envio).toFixed(2);
                                var pago_cliente = Math.round(value.order_total + value.shipping_charge);

                                $("#pago_rest").text(pago_restaurante);
                                $("#costo_envio").text(value.shipping_charge);
                                $("#total_cust").text(pago_c);
                                $("#cambio").text(value.pay_change);
                            });
                            
                            sumPR = (sumPR + parseFloat(value.pago_restaurante));
                            sumE = (sumE + parseFloat(value.envio));
                            sumCR = (sumCR + parseFloat(value.comision_restaurante));
                            sumCE = (sumCE + parseFloat(value.comision_envio));
                            sumTC = (sumTC + parseFloat(totalC));
                            sumTV = (sumTV + parseFloat(value.total));
                            sumNP = (sumNP + parseInt(value.estado));
                        });

                        $("#tablaReporte>tbody").append("<tr><th style='text-align:right;'>Total</th><td>$"+sumPR.toFixed(2)+"</td><td>$"+sumE.toFixed(2)+"</td><td>$"+sumCR.toFixed(2)+"</td><td>$"+sumCE.toFixed(2)+"</td><td>$ "+sumTC.toFixed(2)+"</td><td>$ "+sumTV.toFixed(2)+"</td><td>"+sumNP+"</td></tr>");
                    }

                    /*Count pedidos entregados
                    alert(result.entregadas.length);
                    var countP = 0;
                    var totalPed = 0;
                    $.each(result.entregadas, function(index, value){
                        if(value.estado == 'Entregada'){
                            totalPed = (totalPed + parseInt(1));
                        }
                    });
                        $("#tablaReporte>tbody").append("<tr><th style='text-align:right;'>Pedidos entregados: </th><td colspan='6'>"+totalPed+"</td></tr>");
                    */
                })
                .fail(function (result){
                  alert("Error al consultar los datos");
                });
            }
        });

        //CODIGO PARA EXPORTAR A EXCEL
        $("#exportarRep").on('click', function(){
            var motivo = $("#motivo_busqueda option:selected").val();
            
            if(motivo == "fecha"){
                var fecha1 = $("#datepicker1").val();
                var fecha2 = $("#datepicker2").val();

                var caracter = "T";
                var fecha_ini = fecha1.replace(caracter,' ');
                var fecha_f = fecha2.replace(caracter,' ');

                $.ajax({
                    url: 'exportRep',
                    type: 'post',
                    dataType: 'json',
                    data: {
                       motivo : motivo,
                       fecha_inicio : fecha_ini,
                       fecha_fin : fecha_f  
                    }
                })
                .done(function ( result ){
                    console.log(result);
                    
                })
                .fail(function (result){
                  alert("Error al consultar los datos");
                });
            }else if(motivo == "restaurant"){
                var rest = $("#restauranteSel option:selected").val();
               
                $.ajax({
                    url: 'exportRep',
                    type: 'post',
                    dataType: 'json',
                    data: {
                       motivo : motivo,
                       rest : rest  
                    }
                })
                .done(function ( result ){
                    console.log(result);
                    
                })
                .fail(function (result){
                  alert("Error al consultar los datos");
                });
            }else if(motivo == "mes"){
                var mes = $("#mes_list option:selected").val();
                
                $.ajax({
                    url: 'exportRep',
                    type: 'post',
                    dataType: 'json',
                    data: {
                       motivo : motivo,
                       mes : mes  
                    }
                })
                .done(function ( result ){
                    console.log(result);
                    
                })
                .fail(function (result){
                  alert("Error al consultar los datos");
                });
            }
        });

        //Codigo para reporte IVA
        $("#generarReporteIva").on('click', function(){
            $("#formularioExc").show();

            if($('#cuerpoT').length == 0){
            }else{
                $("#cuerpoT").empty();
            }            
            
            var motivo = $("#motivo_busqueda option:selected").val();
            
            if(motivo == "fecha"){
                var fecha1 = $("#datepicker11").val();
                var fecha2 = $("#datepicker22").val();

                $("#motivoiva").val(motivo);
                $("#fechaiva1").val(fecha1);
                $("#fechaiva2").val(fecha2);

                var caracter = "T";
                var fecha_ini = fecha1.replace(caracter,' ');
                var fecha_f = fecha2.replace(caracter,' ');
                var sumPago = 0;
                var sumIva = 0;
                var sumPagoF = 0;
                var totalIva = 0;

                $.ajax({
                    url: 'datos_iva',
                    type: 'post',
                    dataType: 'json',
                    data: {
                       motivo : motivo,
                       fecha_inicio : fecha_ini,
                       fecha_fin : fecha_f  
                    }
                })
                .done(function ( result ){
                    console.log(result.entregadas);
                    
                    if(result.ordenes.length == ""){
                        $("#tablaReporte>tbody").append("<tr><td colspan='10' style='font-size:15pt; text-align:center;'>No hay resultados de pedidos</td></tr>");
                    }else{
                        $.each(result.ordenes, function(index, value){                            
                            totalC = parseFloat(value.comision_restaurante) + parseFloat(value.comision_envio);
                            totalIva = parseFloat(value.pago_restaurante) * 0.13;
                            totalCIva = parseFloat(value.pago_restaurante) / 1.13;

                            $("#tablaReporte>tbody").append("<tr><td>"+ value.fecha +"</td><td>"+ value.restaurante +"</td><td>$" + value.pago_restaurante + "</td><td>$"+ totalIva.toFixed(2) +"</td><td>$"+ totalCIva.toFixed(2) +"</td></tr>");

                            sumPago = (sumPago + parseFloat(value.pago_restaurante));
                            sumIva = (sumIva + parseFloat(totalIva));
                            sumPagoF = (sumPagoF + parseFloat(totalCIva));
                        });
                        
                        $("#tablaReporte>tbody").append("<tr><th style='text-align:right;'>Total</th><td>&nbsp;</td><td>$"+sumPago.toFixed(2)+"</td><td>$"+sumIva.toFixed(2)+"</td><td>$ "+sumPagoF.toFixed(2)+"</td></tr>");                        
                    }
                })
                .fail(function (result){
                    alert("Error al consultar los datos");
                });        
            }else if(motivo == "mes"){
                var mes = $("#meses1 option:selected").val();
                var anio = $("#anio1 option:selected").val();
                var rest = $("#restauranteSel option:selected").val();
                
                //$("#textoRest").text('* '+ $("#restauranteSel option:selected").text());                
                var sumatoria = 0;
                var sumPago = 0;
                var sumIva = 0;
                var sumPagoF = 0;
                var totalIva = 0;
                var totalIva = 0;

                $("#motivoiva").val(motivo);
                $("#mesiva").val(mes);
                $("#anioiva").val(anio);
                $("#restiva").val(rest);

                console.log(mes +' '+anio);

                $.ajax({
                    url: 'datos_iva',
                    type: 'post',
                    dataType: 'json',
                    data: {
                       motivo : motivo,
                       mes : mes,
                       anio : anio,
                       rest : rest                    
                    }
                })
                .done(function ( result ){
                    console.log(result);
                    
                    if(result.ordenes.length == ""){
                        $("#tablaReporte>tbody").append("<tr><td colspan='10' style='font-size:15pt; text-align:center;'>No hay resultados de pedidos</td></tr>");
                    }else{
                        $.each(result.ordenes, function(index, value){                            
                            totalC = parseFloat(value.comision_restaurante) + parseFloat(value.comision_envio);
                            totalIva = parseFloat(value.pago_restaurante) * 0.13;
                            totalCIva = parseFloat(value.pago_restaurante) / 1.13;

                            $("#tablaReporte>tbody").append("<tr><td>"+ value.fecha +"</td><td>"+ value.restaurante +"</td><td>$" + value.pago_restaurante + "</td><td>$"+ totalIva.toFixed(2) +"</td><td>$"+ totalCIva.toFixed(2) +"</td></tr>");
                            
                            sumPago = (sumPago + parseFloat(value.pago_restaurante));
                            sumIva = (sumIva + parseFloat(totalIva));
                            sumPagoF = (sumPagoF + parseFloat(totalCIva));                            
                        });

                        $("#tablaReporte>tbody").append("<tr><th style='text-align:right;'>Total</th><td>&nbsp;</td><td>$"+sumPago.toFixed(2)+"</td><td>$"+sumIva.toFixed(2)+"</td><td>$ "+sumPagoF.toFixed(2)+"</td></tr>");
                    }
                })
                .fail(function (result){
                  alert("Error al consultar los datos");
                });
            }
        });

        /**************SCRIPTS PARA REPORTES RESTAURANTE CARRIER*************/

        //REPORTE POR FECHAS
        $("#buscar").on('click', function(){
            //var motivo = $("#motivo_busqueda option:selected").val();
            var motivo = "fecha";
            
            if(motivo == "fecha"){
                var fecha1 = $("#datepicker1").val();
                var fecha2 = $("#datepicker2").val();

                var caracter = "T";
                var fecha_ini = fecha1.replace(caracter,' ');
                var fecha_f = fecha2.replace(caracter,' ');
                var sumatoria = 0;

                $.ajax({
                    url: 'busqueda_reporte_rest',
                    type: 'post',
                    dataType: 'json',
                    data: {
                       motivo : motivo,
                       fecha_inicio : fecha_ini,
                       fecha_fin : fecha_f  
                    }
                })
                .done(function ( result ){
                    console.log(result);
                    
                    if(result.length == ""){
                        $("#tablaReporte_rest").append("<tr><td colspan='10' style='font-size:15pt; text-align:center;'>No hay resultados de pedidos</td></tr>");
                    }else{
                        $.each(result, function(index, value){
                            console.log("index"+index);
                            /* Vamos agregando a nuestra tabla las filas necesarias */
                            if(value.payment_method_id == 1){
                                var pago = 'Efectivo';
                            }else if(value.payment_method_id == 2){
                                var pago = 'Tarjeta';
                            }else if(value.payment_method_id == 3){
                                var pago = 'Tigo Money';
                            }

                            if(value.order_status_id == 1){
                                var status = "Pendiente";
                            }else if(value.order_status_id == 2){
                                var status = "Cobrada";
                            }else if(value.order_status_id == 3){
                                var status = "Aceptada";
                            }else if(value.order_status_id == 4){
                                var status = "Despachada";
                            }else if(value.order_status_id == 5){
                                var status = "Completada";
                            }else if(value.order_status_id == 6){
                                var status = "Cancelada";
                            }else if(value.order_status_id == 7){
                                var status = "Cancelada";
                            }else if(value.order_status_id == 8){
                                var status = "Incobrable";
                            }else if(value.order_status_id == 9){
                                var status = "Cancelada";
                            }else if(value.order_status_id == 12){
                                var status = "Pendiente";
                            }else if(value.order_status_id == 13){
                                var status = "Sin Estado";
                            }else if(value.order_status_id == 'Pendiente'){
                                var status = "Sin Estado";
                            }

                            $("#tablaReporte_rest").append("<tr><td>" + value.order_cod + "</td><td>" + value.created_at + "</td><td>" + pago + "</td><td>" + value.address + "</td><td>" + value.zone + "</td><td>" + value.name + "</td><td>" + value.nombre + "</td><td>" + status + "</td><td><button id='order_id_"+ value.order_id +"' class='btn btn-primary test'>Ver detalle</button></td></tr>");
                            
                            $("#order_id_"+value.order_id).on('click', function(){
                                
                                $("#detalleOrden").modal("show");
                                /*Datos del cliente en la orden*/
                                $("#cod_orden").text(value.order_cod);
                                //$("#ord_name").text(value.customer);
                                //$("#ord_address").text(value.address);
                                
                                /*if(value.reference != ''){
                                    $("#ord_ref").text(value.reference);
                                }else{
                                    $("#ord_ref").text('Sin Referencia');
                                }*/

                                if(value.zone != ''){
                                    $("#ord_zone").text(value.zone);
                                }else{
                                    $("#ord_zone").text('Sin Zona');
                                }
                                
                                //Tiempo total de la orden
                                var diferencia = Math.abs(value.tiempoC - value.created_at);
                                var da = value.created_at;
                                var da2 = value.tiempoC;
                                var ultimo = new Date(da.substr(0,4)+"/"+da.substr(5,2)+"/"+da.substr(8,2)+" "+da.substr(11,8));
                                var actual = new Date(da2.substr(0,4)+"/"+da2.substr(5,2)+"/"+da2.substr(8,2)+" "+da2.substr(11,8));
                                var diff=new Date(actual - ultimo);
                                var hora=0;
                                var minutos=0;
                                var segundos=0;
                                if(diff.getUTCHours() <= 9){
                                  hora = "0"+diff.getUTCHours();
                                }else{
                                  hora = diff.getUTCHours();
                                }

                                if(diff.getUTCMinutes() <= 9){
                                  minutos = "0"+diff.getUTCMinutes();
                                }else{
                                  minutos = diff.getUTCMinutes();
                                }

                                if(diff.getUTCSeconds() <= 9){
                                  segundos = "0"+diff.getUTCSeconds();
                                }else{
                                  segundos = diff.getUTCSeconds();
                                }

                                $("#tot_tie").text(hora+":"+minutos+":"+segundos);

                                $("#ord_date").text(value.created_at);
                                $("#ord_total").text('$ '+value.order_total);
                                //$("#tot_tie").text(diferencia);

                                /*Datos de pago*/
                                var com = value.commission_percentage;
                                var comision = ((value.order_total * com) / 100).toFixed(2);
                                var pago_restaurante = (value.order_total - comision).toFixed(2);
                                var total = parseFloat(value.order_total);
                                var envio = parseFloat(value.shipping_charge);
                                var pago_c = (total + envio).toFixed(2);
                                var pago_cliente = Math.round(value.order_total + value.shipping_charge);

                                $("#pago_rest").text(pago_restaurante);
                                $("#costo_envio").text(value.shipping_charge);
                                $("#total_cust").text(pago_c);
                                $("#cambio").text(value.pay_change);
                            });
                            sumatoria = (sumatoria + parseFloat(value.cobro_motoboy));
                        });
                        $("#tablaReporte").append("<tr><td colspan='2' style='text-align:right;'>Total</td><td></td><td></td><td></td><td>$ "+sumatoria.toFixed(2)+"</td><td></td><td></td><td></td><td></td></tr>");
                    }
                })
                .fail(function (result){
                  alert("Error al consultar los datos");
                });

            }else if(motivo == "moto"){
                var motorista = $("#moto option:selected").val();
                var sumatoria = 0;

                $.ajax({
                    url: 'busqueda_reporte',
                    type: 'post',
                    dataType: 'json',
                    data: {
                       motivo : motivo,
                       motorista : motorista  
                    }
                })
                .done(function ( result ){
                    
                    if(result.length == ""){
                        $("#tablaReporte_rest").append("<tr><td colspan='10' style='font-size:15pt; text-align:center;'>No hay resultados de pedidos</td></tr>");
                    }else{
                        $.each(result, function(index, value){
                            /* Vamos agregando a nuestra tabla las filas necesarias */
                            
                            if(value.payment_method_id == 1){
                                var pago = 'Efectivo';
                            }else if(value.payment_method_id == 2){
                                var pago = 'Tarjeta';
                            }else if(value.payment_method_id == 3){
                                var pago = 'Tigo Money';
                            }

                            if(value.order_status_id == 1){
                                var status = "Pendiente";
                            }else if(value.order_status_id == 2){
                                var status = "Cobrada";
                            }else if(value.order_status_id == 3){
                                var status = "Aceptada";
                            }else if(value.order_status_id == 4){
                                var status = "Despachada";
                            }else if(value.order_status_id == 5){
                                var status = "Completada";
                            }else if(value.order_status_id == 6){
                                var status = "Cancelada";
                            }else if(value.order_status_id == 7){
                                var status = "Cancelada";
                            }else if(value.order_status_id == 8){
                                var status = "Incobrable";
                            }else if(value.order_status_id == 9){
                                var status = "Cancelada";
                            }else if(value.order_status_id == 12){
                                var status = "Completada";
                            }else if(value.order_status_id == 13){
                                var status = "Sin Estado";
                            }

                            $("#tablaReporte_rest").append("<tr><td>" + value.order_cod + "</td><td>" + value.created_at + "</td><td>" + pago + "</td><td>" + value.address + "</td><td>" + value.zone + "</td><td>$" + value.cobro_motoboy + "</td><td>" + value.name + "</td><td>" + value.nombre + "</td><td>" + status + "</td><td><button id='order_id_"+ value.order_id +"' class='btn btn-primary test'>Ver detalle</button></td></tr>");

                            $("#order_id_"+value.order_id).on('click', function(){
                                
                                $("#detalleOrden").modal("show");
                                /*Datos del cliente en la orden*/
                                $("#cod_orden").text(value.order_cod);
                                //$("#ord_name").text(value.customer);
                                //$("#ord_address").text(value.address);
                                
                                /*if(value.reference != ''){
                                    $("#ord_ref").text(value.reference);
                                }else{
                                    $("#ord_ref").text('Sin Referencia');
                                }*/

                                if(value.zone != ''){
                                    $("#ord_zone").text(value.zone);
                                }else{
                                    $("#ord_zone").text('Sin Zona');
                                }

                                //Tiempo total de la orden
                                var diferencia = Math.abs(value.tiempoC - value.created_at);
                                var da = value.created_at;
                                var da2 = value.tiempoC;
                                var ultimo = new Date(da.substr(0,4)+"/"+da.substr(5,2)+"/"+da.substr(8,2)+" "+da.substr(11,8));
                                var actual = new Date(da2.substr(0,4)+"/"+da2.substr(5,2)+"/"+da2.substr(8,2)+" "+da2.substr(11,8));
                                var diff=new Date(actual - ultimo);
                                var hora=0;
                                var minutos=0;
                                var segundos=0;
                                if(diff.getUTCHours() <= 9){
                                  hora = "0"+diff.getUTCHours();
                                }else{
                                  hora = diff.getUTCHours();
                                }

                                if(diff.getUTCMinutes() <= 9){
                                  minutos = "0"+diff.getUTCMinutes();
                                }else{
                                  minutos = diff.getUTCMinutes();
                                }

                                if(diff.getUTCSeconds() <= 9){
                                  segundos = "0"+diff.getUTCSeconds();
                                }else{
                                  segundos = diff.getUTCSeconds();
                                }

                                $("#tot_tie").text(hora+":"+minutos+":"+segundos);

                                $("#ord_date").text(value.created_at);
                                $("#ord_total").text('$ '+value.order_total);

                                /*Datos de pago*/
                                var com = value.commission_percentage;
                                var comision = ((value.order_total * com) / 100).toFixed(2);
                                var pago_restaurante = (value.order_total - comision).toFixed(2);
                                var total = parseFloat(value.order_total);
                                var envio = parseFloat(value.shipping_charge);
                                var pago_c = (total + envio).toFixed(2);
                                var pago_cliente = Math.round(value.order_total + value.shipping_charge);

                                $("#pago_rest").text(pago_restaurante);
                                $("#costo_envio").text(value.shipping_charge);
                                $("#total_cust").text(pago_c);
                                $("#cambio").text(value.pay_change);
                            });
                            sumatoria = (sumatoria + parseFloat(value.cobro_motoboy));
                        });
                        $("#tablaReporte").append("<tr><td colspan='2' style='text-align:right;'>Total</td><td></td><td></td><td></td><td>$ "+sumatoria.toFixed(2)+"</td><td></td><td></td><td></td><td></td></tr>");
                    }
                })
                .fail(function (result){
                    alert("Error al consultar los datos");
                });

            }else if(motivo == "pago"){
                var pago = $("#tipo_pago option:selected").val();
                var sumatoria = 0;

                $.ajax({
                    url: 'busqueda_reporte',
                    type: 'post',
                    dataType: 'json',
                    data: {
                       motivo : motivo,
                       pago : pago
                    }
                })
                .done(function (result){

                    if(result.length == ""){
                        $("#tablaReporte_rest").append("<tr><td colspan='10' style='font-size:15pt; text-align:center;'>No hay resultados de pedidos</td></tr>");
                    }else{
                        $.each(result, function(index, value){
                            /* Vamos agregando a nuestra tabla las filas necesarias */
                            if(value.payment_method_id == 1){
                                var pago = 'Efectivo';
                            }else if(value.payment_method_id == 2){
                                var pago = 'Tarjeta';
                            }else if(value.payment_method_id == 3){
                                var pago = 'Tigo Money';
                            }

                            if(value.order_status_id == 1){
                                var status = "Pendiente";
                            }else if(value.order_status_id == 2){
                                var status = "Cobrada";
                            }else if(value.order_status_id == 3){
                                var status = "Aceptada";
                            }else if(value.order_status_id == 4){
                                var status = "Despachada";
                            }else if(value.order_status_id == 5){
                                var status = "Completada";
                            }else if(value.order_status_id == 6){
                                var status = "Cancelada";
                            }else if(value.order_status_id == 7){
                                var status = "Cancelada";
                            }else if(value.order_status_id == 8){
                                var status = "Incobrable";
                            }else if(value.order_status_id == 9){
                                var status = "Cancelada";
                            }else if(value.order_status_id == 12){
                                var status = "Completada";
                            }else if(value.order_status_id == 13){
                                var status = "Sin Estado";
                            }
                            
                            $("#tablaReporte_rest").append("<tr><td>" + value.order_cod + "</td><td>" + value.created_at + "</td><td>" + pago + "</td><td>" + value.address + "</td><td>" + value.zone + "</td><td>$" + value.cobro_motoboy + "</td><td>" + value.name + "</td><td>" + value.nombre + "</td><td>" + status + "</td><td><button id='order_id_"+ value.order_id +"' class='btn btn-primary test'>Ver detalle</button></td></tr>");

                            $("#order_id_"+value.order_id).on('click', function(){
                                
                                $("#detalleOrden").modal("show");
                                /*Datos del cliente en la orden*/
                                $("#cod_orden").text(value.order_cod);
                                //$("#ord_name").text(value.customer);
                                //$("#ord_address").text(value.address);
                                
                                /*if(value.reference != ''){
                                    $("#ord_ref").text(value.reference);
                                }else{
                                    $("#ord_ref").text('Sin Referencia');
                                }*/

                                if(value.zone != ''){
                                    $("#ord_zone").text(value.zone);
                                }else{
                                    $("#ord_zone").text('Sin Zona');
                                }
                                
                                //Tiempo total de la orden
                                var diferencia = Math.abs(value.tiempoC - value.created_at);
                                var da = value.created_at;
                                var da2 = value.tiempoC;
                                var ultimo = new Date(da.substr(0,4)+"/"+da.substr(5,2)+"/"+da.substr(8,2)+" "+da.substr(11,8));
                                var actual = new Date(da2.substr(0,4)+"/"+da2.substr(5,2)+"/"+da2.substr(8,2)+" "+da2.substr(11,8));
                                var diff=new Date(actual - ultimo);
                                var hora=0;
                                var minutos=0;
                                var segundos=0;
                                if(diff.getUTCHours() <= 9){
                                  hora = "0"+diff.getUTCHours();
                                }else{
                                  hora = diff.getUTCHours();
                                }

                                if(diff.getUTCMinutes() <= 9){
                                  minutos = "0"+diff.getUTCMinutes();
                                }else{
                                  minutos = diff.getUTCMinutes();
                                }

                                if(diff.getUTCSeconds() <= 9){
                                  segundos = "0"+diff.getUTCSeconds();
                                }else{
                                  segundos = diff.getUTCSeconds();
                                }

                                $("#tot_tie").text(hora+":"+minutos+":"+segundos);
                                
                                $("#ord_date").text(value.created_at);
                                $("#ord_total").text('$ '+value.order_total);

                                /*Datos de pago*/
                                var com = value.commission_percentage;
                                var comision = ((value.order_total * com) / 100).toFixed(2);
                                var pago_restaurante = (value.order_total - comision).toFixed(2);
                                var total = parseFloat(value.order_total);
                                var envio = parseFloat(value.shipping_charge);
                                var pago_c = (total + envio).toFixed(2);
                                var pago_cliente = Math.round(value.order_total + value.shipping_charge);

                                $("#pago_rest").text(pago_restaurante);
                                $("#costo_envio").text(value.shipping_charge);
                                $("#total_cust").text(pago_c);
                                $("#cambio").text(value.pay_change);
                            });
                            
                            sumatoria = (sumatoria + parseFloat(value.cobro_motoboy));
                        });
                        $("#tablaReporte").append("<tr><td colspan='2' style='text-align:right;'>Total</td><td></td><td></td><td></td><td>$ "+sumatoria.toFixed(2)+"</td><td></td><td></td><td></td><td></td></tr>");
                    }
                })
                .fail(function (result){
                    alert("Error al consultar los datos");
                });
            }
        });

        //REPORTE DE VENTAS POR FECHAS Y POR MES
        $("#generarReporteV_rest").on('click', function(){
            //$("#formularioExc").show();

            if($('#cuerpoT').length == 0){
            }else{
                $("#cuerpoT").empty();
            }            
            
            var motivo = $("#motivo_busqueda option:selected").val();
            
            if(motivo == "fecha"){
                var fecha1 = $("#datepicker1").val();
                var fecha2 = $("#datepicker2").val();

                $("#motivo2").val(motivo);
                $("#fechaN1").val(fecha1);
                $("#fechaN2").val(fecha2);

                var caracter = "T";
                var fecha_ini = fecha1.replace(caracter,' ');
                var fecha_f = fecha2.replace(caracter,' ');
                var sumPR = 0;
                var sumE = 0;
                var sumCR = 0;
                var sumCE = 0;
                var sumTC = 0;
                var sumTV = 0;
                var totalC = 0;
                var sumNP = 0;

                $.ajax({
                    url: 'ventaRep_rest',
                    type: 'post',
                    dataType: 'json',
                    data: {
                       motivo : motivo,
                       fecha_inicio : fecha_ini,
                       fecha_fin : fecha_f  
                    }
                })
                .done(function ( result ){
                    console.log(result.entregadas);
                    
                    if(result.ordenes.length == ""){
                        $("#tablaReporteV>tbody").append("<tr><td colspan='10' style='font-size:15pt; text-align:center;'>No hay resultados de pedidos</td></tr>");
                    }else{
                        $.each(result.ordenes, function(index, value){
                            console.log("index"+index);
                            totalC = parseFloat(value.comision_restaurante) + parseFloat(value.comision_envio);
                            
                            $("#tablaReporteV>tbody").append("<tr><td>" + value.fecha + "</td><td>$" + value.pago_restaurante + "</td><td>" + value.estado + "</td></tr>");
                            
                            $("#order_id_"+value.order_id).on('click', function(){
                                
                                $("#detalleOrden").modal("show");
                                /*Datos del cliente en la orden*/
                                $("#cod_orden").text(value.order_cod);
                            
                                if(value.zone != ''){
                                    $("#ord_zone").text(value.zone);
                                }else{
                                    $("#ord_zone").text('Sin Zona');
                                }
                                
                                //Tiempo total de la orden
                                var diferencia = Math.abs(value.tiempoC - value.created_at);
                                var da = value.created_at;
                                var da2 = value.tiempoC;
                                var ultimo = new Date(da.substr(0,4)+"/"+da.substr(5,2)+"/"+da.substr(8,2)+" "+da.substr(11,8));
                                var actual = new Date(da2.substr(0,4)+"/"+da2.substr(5,2)+"/"+da2.substr(8,2)+" "+da2.substr(11,8));
                                var diff=new Date(actual - ultimo);
                                var hora=0;
                                var minutos=0;
                                var segundos=0;

                                if(diff.getUTCHours() <= 9){
                                  hora = "0"+diff.getUTCHours();
                                }else{
                                  hora = diff.getUTCHours();
                                }

                                if(diff.getUTCMinutes() <= 9){
                                  minutos = "0"+diff.getUTCMinutes();
                                }else{
                                  minutos = diff.getUTCMinutes();
                                }

                                if(diff.getUTCSeconds() <= 9){
                                  segundos = "0"+diff.getUTCSeconds();
                                }else{
                                  segundos = diff.getUTCSeconds();
                                }

                                $("#tot_tie").text(hora+":"+minutos+":"+segundos);

                                $("#ord_date").text(value.created_at);
                                $("#ord_total").text('$ '+value.order_total);
                                //$("#tot_tie").text(diferencia);

                                /*Datos de pago*/
                                var com = value.commission_percentage;
                                var comision = ((value.order_total * com) / 100).toFixed(2);
                                var pago_restaurante = (value.order_total - comision).toFixed(2);
                                var total = parseFloat(value.order_total);
                                var envio = parseFloat(value.shipping_charge);
                                var pago_c = (total + envio).toFixed(2);
                                var pago_cliente = Math.round(value.order_total + value.shipping_charge);

                                $("#pago_rest").text(pago_restaurante);
                                $("#costo_envio").text(value.shipping_charge);
                                $("#total_cust").text(pago_c);
                                $("#cambio").text(value.pay_change);
                            });
                            sumPR = (sumPR + parseFloat(value.pago_restaurante));
                            sumE = (sumE + parseFloat(value.envio));
                            sumCR = (sumCR + parseFloat(value.comision_restaurante));
                            sumCE = (sumCE + parseFloat(value.comision_envio));
                            sumTC = (sumTC + parseFloat(totalC));
                            sumTV = (sumTV + parseFloat(value.total));
                            sumNP = (sumNP + parseInt(value.estado));
                        });

                        $("#tablaReporteV>tbody").append("<tr><th style='text-align:right;'>Total</th><td>$"+sumPR.toFixed(2)+"</td><td>"+sumNP+"</td></tr>");
                    }

                    /*Count pedidos entregados
                    var countP = 0;
                    var totalPed = 0;
                    $.each(result.entregadas, function(index, value){
                        if(value.estado == 'Entregada'){
                            totalPed = (totalPed + parseInt(1));
                        }
                    });
                    $("#tablaReporte>tbody").append("<tr><th style='text-align:right;'>Pedidos entregados: </th><td colspan='6'>"+totalPed+"</td></tr>");
                    */
                })
                .fail(function (result){
                  alert("Error al consultar los datos");
                });
            }else if(motivo == "mes"){
                var mes = $("#meses option:selected").val();
                var anio = $("#anio option:selected").val();
                var rest = $("#restauranteSel option:selected").val();
                
                //$("#textoRest").text('* '+ $("#restauranteSel option:selected").text());
                
                var sumatoria = 0;
                var sumPR = 0;
                var sumE = 0;
                var sumCR = 0;
                var sumCE = 0;
                var sumTC = 0;
                var sumTV = 0;
                var totalC = 0;
                var sumNP = 0;

                $("#mesN").val(mes);
                $("#AnioN").val(anio);
                //$("#RestN").val(rest);

                //console.log(mes +' '+anio +' '+ rest);

                $.ajax({
                    url: 'ventaRep_rest',
                    type: 'post',
                    dataType: 'json',
                    data: {
                       motivo : motivo,
                       mes : mes,
                       anio : anio                       
                    }
                })
                .done(function ( result ){
                    console.log(result);
                    
                    if(result.ordenes.length == ""){
                        $("#tablaReporteV>tbody").append("<tr><td colspan='10' style='font-size:15pt; text-align:center;'>No hay resultados de pedidos</td></tr>");
                    }else{
                        $.each(result.ordenes, function(index, value){
                            console.log("index"+index);
                            totalC = parseFloat(value.comision_restaurante) + parseFloat(value.comision_envio);                            
                            $("#tablaReporteV>tbody").append("<tr><td>" + value.fecha + "</td><td>$" + value.pago_restaurante + "</td><td>" + value.estado + "</td></tr>");
                            $("#order_id_"+value.order_id).on('click', function(){
                                
                                $("#detalleOrden").modal("show");
                                /*Datos del cliente en la orden*/
                                $("#cod_orden").text(value.order_cod);
                            
                                if(value.zone != ''){
                                    $("#ord_zone").text(value.zone);
                                }else{
                                    $("#ord_zone").text('Sin Zona');
                                }
                                
                                //Tiempo total de la orden
                                var diferencia = Math.abs(value.tiempoC - value.created_at);
                                var da = value.created_at;
                                var da2 = value.tiempoC;
                                var ultimo = new Date(da.substr(0,4)+"/"+da.substr(5,2)+"/"+da.substr(8,2)+" "+da.substr(11,8));
                                var actual = new Date(da2.substr(0,4)+"/"+da2.substr(5,2)+"/"+da2.substr(8,2)+" "+da2.substr(11,8));
                                var diff=new Date(actual - ultimo);
                                var hora=0;
                                var minutos=0;
                                var segundos=0;

                                if(diff.getUTCHours() <= 9){
                                  hora = "0"+diff.getUTCHours();
                                }else{
                                  hora = diff.getUTCHours();
                                }

                                if(diff.getUTCMinutes() <= 9){
                                  minutos = "0"+diff.getUTCMinutes();
                                }else{
                                  minutos = diff.getUTCMinutes();
                                }

                                if(diff.getUTCSeconds() <= 9){
                                  segundos = "0"+diff.getUTCSeconds();
                                }else{
                                  segundos = diff.getUTCSeconds();
                                }

                                $("#tot_tie").text(hora+":"+minutos+":"+segundos);

                                $("#ord_date").text(value.created_at);
                                $("#ord_total").text('$ '+value.order_total);
                                //$("#tot_tie").text(diferencia);

                                /*Datos de pago*/
                                var com = value.commission_percentage;
                                var comision = ((value.order_total * com) / 100).toFixed(2);
                                var pago_restaurante = (value.order_total - comision).toFixed(2);
                                var total = parseFloat(value.order_total);
                                var envio = parseFloat(value.shipping_charge);
                                var pago_c = (total + envio).toFixed(2);
                                var pago_cliente = Math.round(value.order_total + value.shipping_charge);

                                $("#pago_rest").text(pago_restaurante);
                                $("#costo_envio").text(value.shipping_charge);
                                $("#total_cust").text(pago_c);
                                $("#cambio").text(value.pay_change);
                            });
                            
                            sumPR = (sumPR + parseFloat(value.pago_restaurante));
                            sumE = (sumE + parseFloat(value.envio));
                            sumCR = (sumCR + parseFloat(value.comision_restaurante));
                            sumCE = (sumCE + parseFloat(value.comision_envio));
                            sumTC = (sumTC + parseFloat(totalC));
                            sumTV = (sumTV + parseFloat(value.total));
                            sumNP = (sumNP + parseInt(value.estado));
                        });

                        $("#tablaReporteV>tbody").append("<tr><th style='text-align:right;'>Total</th><td>$"+sumPR.toFixed(2)+"</td><td>"+sumNP+"</td></tr>");
                    }

                    /*Count pedidos entregados
                    alert(result.entregadas.length);
                    var countP = 0;
                    var totalPed = 0;
                    $.each(result.entregadas, function(index, value){
                        if(value.estado == 'Entregada'){
                            totalPed = (totalPed + parseInt(1));
                        }
                    });
                        $("#tablaReporte>tbody").append("<tr><th style='text-align:right;'>Pedidos entregados: </th><td colspan='6'>"+totalPed+"</td></tr>");
                    */
                })
                .fail(function (result){
                  alert("Error al consultar los datos");
                });
            }
        });

        //SCRIPT PARA REPORTE DE VENTA DETALLADA
        $("#BVentaDetalle_rest").on('click', function(){

            //$("#formularioExc").show();

            if ($('#cuerpoT').length == 0){
            }else{
                $("#cuerpoT").empty();
            }
            
            var motivo = "fecha";
            
            if(motivo == "fecha"){
                var fecha1 = $("#datepicker1").val();
                var fecha2 = $("#datepicker2").val();

                $("#motivo2").val(motivo);
                $("#fechaN1").val(fecha1);
                $("#fechaN2").val(fecha2);

                var caracter = "T";
                var fecha_ini = fecha1.replace(caracter,' ');
                var fecha_f = fecha2.replace(caracter,' ');
                var sumatoria = 0;
                var Ssubtotal = 0;
                var Senvio = 0;
                var Starjeta = 0;
                var Stotal = 0;
                

                $.ajax({
                    url: 'rep_detalle_rest',
                    type: 'post',
                    dataType: 'json',
                    data: {
                       motivo : motivo,
                       fecha_inicio : fecha_ini,
                       fecha_fin : fecha_f  
                    }
                })
                .done(function ( result ){
                    console.log(result);
                    
                    if(result.length == ""){
                        $("#tablaReporte_rest>tbody").append("<tr><td colspan='10' style='font-size:15pt; text-align:center;'>No hay resultados de pedidos</td></tr>");
                    }else{
                        $.each(result, function(index, value){
                            console.log("index"+index);                            
                            
                            $("#tablaReporte_rest>tbody").append("<tr><td>" + value.order_cod + "</td><td>" + value.fecha + "</td><td>" + value.estado + "</td><td>" + value.tipo_pago + "</td><td>$" + value.pago_restaurante + "</td><td><button id='order_id_"+ value.order_id +"' class='btn btn-primary test'>Ver detalle</button></td></tr>");
                            
                            $("#order_id_"+value.order_id).on('click', function(){
                                
                                $("#detalleOrden_rest").modal("show");
                                /*Datos del cliente en la orden*/
                                $("#cod_orden_r").text(value.order_cod);
                            
                                if(value.zone != ''){
                                    $("#ord_zone_r").text(value.zone);
                                }else{
                                    $("#ord_zone_r").text('Sin Zona');
                                }
                                console.log('TIEMPOC '+value.tiempoC);
                                //Tiempo total de la orden
                                var diferencia = Math.abs(value.tiempoC - value.fecha);
                                var da = value.fecha;
                                var da2 = value.tiempoC;
                                var ultimo = new Date(da.substr(0,4)+"/"+da.substr(5,2)+"/"+da.substr(8,2)+" "+da.substr(11,8));
                                var actual = new Date(da2.substr(0,4)+"/"+da2.substr(5,2)+"/"+da2.substr(8,2)+" "+da2.substr(11,8));
                                var diff=new Date(actual - ultimo);
                                var hora=0;
                                var minutos=0;
                                var segundos=0;

                                if(diff.getUTCHours() <= 9){
                                  hora = "0"+diff.getUTCHours();
                                }else{
                                  hora = diff.getUTCHours();
                                }

                                if(diff.getUTCMinutes() <= 9){
                                  minutos = "0"+diff.getUTCMinutes();
                                }else{
                                  minutos = diff.getUTCMinutes();
                                }

                                if(diff.getUTCSeconds() <= 9){
                                  segundos = "0"+diff.getUTCSeconds();
                                }else{
                                  segundos = diff.getUTCSeconds();
                                }

                                $("#tot_tie_r").text(hora+":"+minutos+":"+segundos);

                                $("#ord_date_r").text(value.fecha);
                                $("#ord_total_r").text('$ '+value.order_total);
                                //$("#tot_tie").text(diferencia);

                                /*Datos de pago*/
                                var com = value.commission_percentage;
                                var comision = ((value.order_total * com) / 100).toFixed(2);
                                var pago_restaurante = (value.order_total - comision).toFixed(2);
                                var total = parseFloat(value.order_total);
                                var envio = parseFloat(value.shipping_charge);
                                var pago_c = (total + envio).toFixed(2);
                                var pago_cliente = Math.round(value.order_total + value.shipping_charge);

                                $("#pago_rest_r").text(pago_restaurante);
                                $("#costo_envio_r").text(value.shipping_charge);
                                $("#total_cust_r").text(pago_c);
                                $("#cambio_r").text(value.pay_change);
                            });

                            Ssubtotal = (Ssubtotal + parseFloat(value.subtotal));
                            Senvio = (Senvio + parseFloat(value.costo_envio));
                            Starjeta = (Starjeta + parseFloat(value.cargo_tarjeta));
                            var com2 = value.commission_percentage;
                            var comision2 = ((value.order_total * com2) / 100).toFixed(2);
                            Stotal = (Stotal + parseFloat((value.order_total - comision2).toFixed(2)));
                        });                        
                        $("#tablaReporte_rest>tbody").append("<tr><th>Total</th><td></td><td></td><td></td><td>$" + Stotal.toFixed(2) + "</td><td></td></tr>");
                    }
                })
                .fail(function (result){
                  alert("Error al consultar los datos");
                });
            }
        });

        //SCRIPT PARA MEDIR TIEMPOS DE CADA ORDEN
        $("#Btiempo").on('click', function(){

            //$("#formularioExc").show();

            if ($('#cuerpoT').length == 0){
            }else{
                $("#cuerpoT").empty();
            }
            
            var motivo = "fecha";
            
            if(motivo == "fecha"){
                var fecha1 = $("#datepicker1").val();
                var fecha2 = $("#datepicker2").val();

                $("#motivo2").val(motivo);
                $("#fechaN1").val(fecha1);
                $("#fechaN2").val(fecha2);

                var caracter = "T";
                var fecha_ini = fecha1.replace(caracter,' ');
                var fecha_f = fecha2.replace(caracter,' ');
                var sumatoria = 0;
                var Ssubtotal = 0;
                var Senvio = 0;
                var Starjeta = 0;
                var Stotal = 0;                

                $.ajax({
                    url: 'rep_tiempos_rest',
                    type: 'post',
                    dataType: 'json',
                    data: {
                       motivo : motivo,
                       fecha_inicio : fecha_ini,
                       fecha_fin : fecha_f  
                    }
                })
                .done(function ( result ){
                    console.log(result);
                    var arreglo = _.groupBy(result, 'orden');
                    console.log(arreglo);
                    if(result.length == ""){
                        $("#tablaReporteT_rest>tbody").append("<tr><td colspan='10' style='font-size:15pt; text-align:center;'>No hay resultados de pedidos</td></tr>");
                    }else{
                        $.each(arreglo, function(index, value){
                            //console.log(value[0]);
                            //console.log(_.isEmpty(value[0]));
                            
                            if(_.isEmpty(value[0])){
                                var hora_llegada = 'Hora no definida';
                            }else{
                                if(typeof value[0].hora_cambio === "undefined"){
                                    var hora_llegada = 'Hora no definida2';
                                }else{
                                    var hora_llegada = value[0].hora_cambio;
                                }    
                            }

                            if(_.isEmpty(value[1])){
                                var hora_salida = 'Hora no definida';
                            }else{
                                if(typeof value[1].hora_cambio === "undefined"){
                                    var hora_salida = 'Hora no definida2';
                                }else{
                                    var hora_salida = value[1].hora_cambio;
                                }
                            }
                            
                            if(_.isEmpty(value[2])){
                                var hora_destino = 'Hora no definida';
                            }else{
                                if(typeof value[2].hora_cambio === "undefined"){
                                    var hora_destino = 'Hora no definida2';
                                }else{
                                    var hora_destino = value[2].hora_cambio;
                                }
                            }
                                                        
                            $("#tablaReporteT_rest>tbody").append("<tr class = 'ord_"+index+"'><td>" + index + "</td><td>"+value[0].hora_creacion+"</td><td>"+hora_llegada+"</td><td>"+hora_salida+"</td><td>"+hora_destino+"</td></tr>");
                        });
                    }
                })
                .fail(function (result){
                  alert("Error al consultar los datos");
                });
            }
        });


        //Aquí comienza el reporte hecho el 6 de abril**************
        //Codigo para reporte por restaurante, tipo pago y rango de fechas  - 06 abril
        $("#generar_reporte_rest_pago").on('click', function(e){
            
            $("#formularioExc").show();

            if($('#cuerpoT').length == 0){
            }else{
                $("#cuerpoT").empty();
            }
            
            //var motivo = $("#motivo_busqueda option:selected").val();
            
            var fecha1 = $("#datepicker_r").val();
            var fecha2 = $("#datepicker_r2").val();
            var restaurante = $("#restauranteSel option:selected").val();
            var tipo_pago = $("#tipo_pago option:selected").val();


            if(restaurante == 'null'){
                alert("Debe elegir un restaurante");
                e.preventDefault();
            }else{
                $("#restExp").val(restaurante);
                $("#fecha1Exp").val(fecha1);
                $("#fecha2Exp").val(fecha2);
                $("#pagoExp").val(tipo_pago);

                var caracter = "T";
                var fecha_ini = fecha1.replace(caracter,' ');
                var fecha_f = fecha2.replace(caracter,' ');
                var sumMT = 0;
                var sumME = 0;
                var sumCP = 0;
                var sumPR = 0;

                console.log(fecha_ini+ "-" +fecha_f+ "-" +restaurante+ "-" +tipo_pago);

                $.ajax({
                    url: 'datos_rest_pago',
                    type: 'post',
                    dataType: 'json',
                    data: {                   
                       fecha_inicio : fecha_ini,
                       fecha_fin : fecha_f,
                       restaurante : restaurante,
                       pago : tipo_pago
                    }
                })
                .done(function (result){
                    console.log(result);
                    
                    if(result.ordenes.length == ""){
                        $("#tablaReporte>tbody").append("<tr><td colspan='10' style='font-size:15pt; text-align:center;'>No hay resultados de pedidos</td></tr>");
                    }else{
                        $.each(result.ordenes, function(index, value){                        

                            $("#tablaReporte>tbody").append("<tr><td>"+ value.fecha +"</td><td>"+ value.order_cod +"</td><td>"+ value.restaurante +"</td><td>"+ value.tipo_pago +"</td><td>$" + value.total + "</td><td>$"+ value.costo_envio +"</td><td>$"+ value.comision_pf +"</td><td>$" + value.pago_restaurante + "</td></tr>");

                            sumMT = (sumMT + parseFloat(value.total));
                            sumME = (sumME + parseFloat(value.costo_envio));
                            sumCP = (sumCP + parseFloat(value.comision_pf));
                            sumPR = (sumPR + parseFloat(value.pago_restaurante));                        
                        });

                        $("#tablaReporte>tbody").append("<tr><th style='text-align:right;'>Total</th><td></td><td></td><td></td><td>$"+sumMT.toFixed(2)+"</td><td>$"+sumME.toFixed(2)+"</td><td>$"+sumCP.toFixed(2)+"</td><td>$"+sumPR.toFixed(2)+"</td></tr>");

                    }
                })
                .fail(function (result){
                    console.log(result);
                    alert("Error al consultar los datos");
                });
            }           
        });
       
        </script>
    </body>
</html>