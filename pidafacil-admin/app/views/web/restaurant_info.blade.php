	...
	<div class="col-lg-9 col-xs-12">
		<div class="row">
			<div class="col-lg-4 col-md-4 col-sm-4 col-xs-12" align="center">
				<a href="{{ URL::to($restaurant_info->slug) }}">{{ HTML::image($logo,'',array('style'=>'height: 100px;')) }}</a>
			</div>
			...
			<h1>Horarios</h1>
                        <table class="table text-center">
                            ...
                            <tr>
                                <th>{{ $dia }}</th>
                                @foreach($service_types as $service_type)
                                <td>{{ date("g:ha", strtotime($schedules[$k][$service_type->service_type_id]['opening'])) }}</td>
                                <td>{{ date("g:ha", strtotime($schedules[$k][$service_type->service_type_id]['closing'])) }}</td>
                                @endforeach
                            </tr>
                            @endforeach
                        </table>
