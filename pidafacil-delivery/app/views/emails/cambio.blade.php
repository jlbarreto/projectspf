...
<body style="background-color: #F6F6F6; padding: 10px;">
    <div style="background-color: #FFF; font-family: Arial, Helvetica, sans-serif; font-size: 12px; padding:10px; border-radius: 10px;">
        <img style="max-height: 80px;" src="http://images.pf.techmov.co/pidafacil_logo.png" />
        <p style="color:#474843; font-size: 1.5em;"><strong>Hola {{ $bmail['user_name'] }},</strong> tu orden <strong>#{{ $bmail['order_cod'] }}</strong> ha sido @if($bmail['status'] == 3 && $bmail['service_type'] !== 2)aceptada y se encuentra en proceso de entrega. @elseif($bmail['status'] == 3 && $bmail['service_type'] == 2)aceptada, puedes recoger tu pedido a la hora indicada. @elseif($bmail['status'] == 6) cancelada. @elseif($bmail['status'] == 7) rechazada. @elseif($bmail['status'] == 8) declarada como incobrable. @endif</p>
        @if($bmail['status'] == 3 )
